...
public class StudentReportPDF {
	...
	public int generatePdf(ArrayList studentList, String sessionid,
		...
		try {
			...
			try {
				...
				// fileName=storagepath+"PDFReports/Student List for
				...
				String fileLoc = storagepath + instituteid + "/archivalpdfs/"
						+ sessionname + "_" + sessionid + "/Student List";
				...
				// tempPath=storagepath+instituteid+"/archivalpdfs/"+sessionname+"_"+sessionid+"/Student
				// List"+"/Student List "+time_stamp+"_temp.pdf";
				tempPath = storagepath + instituteid + "/archivalpdfs/"
						+ sessionname + "_" + sessionid + "/Student List"
						+ "/output.pdf";
				fileName = storagepath + instituteid + "/archivalpdfs/"
						+ sessionname + "_" + sessionid + "/Student List"
						+ "/Student List " + time_stamp + ".pdf";

				// temppath=storagepath+instituteid+"/archivalpdfs/"+sessionname+"_"+reg_sessionid+"/ExamDetails"+"/ExamDetails
				// "+time_stamp+"_temp.pdf";
				// filepath=storagepath+instituteid+"/archivalpdfs/"+sessionname+"_"+reg_sessionid+"/ExamDetails"+"/ExamDetails
				// "+time_stamp+".pdf" ;

				// PdfWriter writer = PdfWriter.getInstance(document,new
				// FileOutputStream(storagepath+"PDFReports/studentList1.pdf"));
				PdfWriter writer = PdfWriter.getInstance(document,
						new FileOutputStream(tempPath));
				document.open();
				Paragraph p3 = new Paragraph("Student List", new Font(
				...
				// p.add(new Chunk("Student List for the session :"+sessionname+"\t", new
				...
				if (studentList != null && studentList.size() > 0) {
					for (int i = 0, j = 1; i < studentList.size(); i++) {
						...
						stdSchedId = (String) applicantListMap
								.get("standardscheduleid");
						applicantName = (String) applicantListMap
								.get("applicantname");
						rollNo = (String) applicantListMap.get("rollno");
						standardName = (String) applicantListMap
								.get("standardname");
			...
			StudentReportPDF pagenooo = new StudentReportPDF();
	...
	public void onEverypage(String s, String s1, Document document,
		...
		try {
			...
			while (i < n) {
				 ...
				 * over.showText("Student List"); over.setFontAndSize(bf, 2f);
				 * over.endText();
				 */
				try {
