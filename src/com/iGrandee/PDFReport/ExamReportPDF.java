...
public class ExamReportPDF {
	...
	public int generatePdf(ArrayList examAllotList, String sessionid,
			String sessionname, String boardname, String instituteid) {

		// com.iGrandee.Fees.FeesSchedule feesscheduleQuery = new
		// com.iGrandee.Fees.FeesSchedule();
		...
		try {
			...
			try {
				...
				// tempPath=storagepath+"PDFReports/examMarkList1.pdf";
				// fileName=storagepath+"PDFReports/Exam Mark List for
				...
				String fileLoc = storagepath + instituteid + "/archivalpdfs/"
						+ sessionname + "_" + sessionid + "/Mark Details";
				...
				fileName = storagepath + instituteid + "/archivalpdfs/"
						+ sessionname + "_" + sessionid + "/Mark Details"
						+ "/Mark Details " + time_stamp + ".pdf";

				// PdfWriter writer = PdfWriter.getInstance(document,new
				// FileOutputStream(storagepath+"PDFReports/examMarkList1.pdf"));
				PdfWriter writer = PdfWriter.getInstance(document,
						new FileOutputStream(tempPath));

				document.open();
				Paragraph p3 = new Paragraph("Mark Details", new Font(
				...
				// p.add(new Chunk("Timesheet status for the date
				// :"+strartdae+"\t"+
				// "to"+"\t"+enddate, new Font(Font.TIMES_ROMAN,
				// 12,Font.BOLD,Color.black)));
				// p.add(new Chunk("Exam Mark List for the session
				// :"+sessionname+"\t", new
				...
				if (examAllotList != null && examAllotList.size() > 0) {
					...
					cell = new PdfPCell(
							new Paragraph("Note : NE - Not Entered", new Font(
					...
					int headerwidths[] = { 5, 21, 10, 11, 20, 15, 10, 7 }; // percentage
					...
					// Color getcolor = new
					// Color(gerrncolor.getRed(),gerrncolor.getGreen(),gerrncolor.getBlue(),150);
					...
					for (int i = 0, j = 1; i < examAllotList.size(); i++) {
						...
						// String termfeesAmount = (String)
						// map.get("termfeesamount");
						// String termName = (String) map.get("termname");
						// String dueDate = (String) map.get("duedate");

						/*
						 * Long termFeeObj = Long.valueOf(termfeesAmount);
						 * NumberFormat termFeeFormat =
						 * NumberFormat.getCurrencyInstance(localcurrency);
						 * formattedTermFeeCurrency =
						 * termFeeFormat.format(termFeeObj);
						 */
						...
						// pendingCountList =
						// (ArrayList)feesscheduleQuery.displayFeesDetailsPdf(standardscheduleId,termfeesId);
						examMarkList = (ArrayList) examAllotmentQuery
								.examMarkforPDF(standardscheduleId,
										examAllotmentId);
						...
						if (examAllotList.size() != 0) {
							...
							for (int k = 0; k < examMarkList.size(); k++) {
								resultStat = "Pass";
								HashMap examMarkMap = (HashMap) examMarkList
										.get(k);
								// System.out.println("examMarkMap-->"+examMarkMap);

								applicantName = (String) examMarkMap
										.get("studentname");
								rollNo = (String) examMarkMap.get("rollno");
								secName = (String) examMarkMap
										.get("sectionname");
								subjectName = (String) examMarkMap
										.get("subjectname");
								subjectCode = (String) examMarkMap
										.get("subjectcode");
								subjectMark = (String) examMarkMap
										.get("marktaken");
								studAllocId = (String) examMarkMap
										.get("studentallocationid");
								passMark = (String) examMarkMap
										.get("theoryexternalmin");
								...
								if (subjectMark == null) {
									...
									try {
										if (Integer.parseInt(subjectMark) < Integer
												.parseInt(passMark))
											resultStat = "Fail";
								...
								// feeStatus =
								// (String)pendingMap.get("feestatus");
								// termFeesAmount =
								// (String)pendingMap.get("termfeesamount");
								// paidAmount =
								// (String)pendingMap.get("FeesPaid");
								// remainAmount =
								// (String)pendingMap.get("RemAmount");
								// String pendingStatus =
								// (String)pendingMap.get("pendingstatus");
								 ...
								 * NumberFormat paidFeeFormat =
								 * NumberFormat.getCurrencyInstance(localcurrency);
								 * formattedPaidCurrency =
								 * paidFeeFormat.format(paidFeeObj);
								 * 
								 * Long pendFeeObj = Long.valueOf(remainAmount);
								 * NumberFormat pendFeeFormat =
								 * NumberFormat.getCurrencyInstance(localcurrency);
								 * formattedPendCurrency =
								 * pendFeeFormat.format(pendFeeObj);
								 */
								...
								// if(!tempStdList.contains(standardscheduleId))
								{
									...
									if (!examAllotIdList
											.contains(examAllotmentId)) {
										studAllocIdList = new ArrayList();
										serialCount = 1;
										// PdfPCell cell2;
										cell1 = new PdfPCell(
												new Paragraph("Exam Name : "
														+ examName, new Font(
														Font.TIMES_ROMAN, 11,
								...
								if (!studAllocIdList.contains(studAllocId)) {
									...
									// String
									// names=timesheetAList.get(9).toString();
									...
									// String
									// roles=timesheetAList.get(5).toString();
								...
								cell = new PdfPCell(new Paragraph(subjectMark));
								cell.setHorizontalAlignment(Element.ALIGN_LEFT);
								table.addCell(cell);

								// cell = new PdfPCell(new Paragraph("Note : NE
								// - Not Entered",
								// new Font(Font.TIMES_ROMAN, 11,Font.BOLD,new
								// Color(gerrncolor.getRed(),gerrncolor.getGreen(),gerrncolor.getBlue(),150))));
								...
								if (resultStat.equals("Fail")) {
									...
									// Paragraph(resultStat,new
									// Font(Font.TIMES_ROMAN,
									// 14,Font.NORMAL,Color.red)));
									cell = new PdfPCell(new Paragraph(
											resultStat, new Font(
													Font.TIMES_ROMAN, 14,
					...
					// colspan='5'><font color='red'>Data Not
					// Found</font></td></tr>");
			...
			ExamReportPDF pagenooo = new ExamReportPDF();
	...
	public void onEverypage(String s, String s1, Document document,
		...
		try {
			...
			while (i < n) {
				 ...
				 * over.showText("Mark Details"); over.setFontAndSize(bf, 2f);
				 * over.endText();
				 */
				try {
...
e.printStackTrace();
	...
	public void onEndPage(PdfWriter writer, Document document,
		...
		try {
			...
			if (insdetails != null && insdetails.size() > 0) {
				...
				head.addCell(new Paragraph("Exam Mark List ", new Font(
	...
	public void pagenumber1(String s, String s1, Document document) {
		 ...
		 * over.setFontAndSize(bf, 10);
		 * //over.setTextMatrix((page.width()-20)/2, 20); over.showText("- " +
