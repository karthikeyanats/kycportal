...
<title>Know Your Child - <%=bundle1.getString("label.feespaidlistdetails.title")%>| Powered by i-Grandee</title>
...
function printfun()
{
	window.open("PrintPreviewFeesdetails.jsp?rollno="+document.feescollectionform.rollnumbertext.value+"&feesdate="+document.feescollectionform.feedate.value+"&disfeesdate="+document.feescollectionform.disfeesdate.value+"&studentapprovalid="+document.feescollectionform.studentapprovalid.value+"","Msg","top=250,left=350,help:no,center:no,status:no,width=730,height=600,scrollbars=1;")
}
function backfun()
{
		document.feescollectionform.action="./FeesPaidListStudent.jsp";
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle1.getString("label.feespaidlistdetails.rootpath")%></h2>
  ...
  <%@ page import="java.util.ResourceBundle,java.util.ArrayList,java.util.HashMap,com.iGrandee.Common.DateTime,java.text.NumberFormat,java.util.Locale"%>
    ...
    // Locale localcurrency = new Locale("en", "IN");
     ...
     try
     {
    	  ...
    	  date=request.getParameter("feesdate");
    	  disfeesdate=request.getParameter("disfeesdate");
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
           	<th scope="col" class="rounded-company" width="90%"><%=bundle1.getString("label.feespaidlistdetails.StudentInformation")%></th>
	...
	//mediumname, firstname, middlename, lastname, emailid, imagepath, prefix,studentapprovalid   		 		
	    	...
	    	<tr><td class=tablebold><%=bundle1.getString("label.feespaidlistdetails.StudentRollNumber")%></td>
	    	<td class=tablebold>:</td><td><%=reqrollnumbertext %></td>
	    	<td class=tablebold><%=bundle1.getString("label.feespaidlistdetails.StudentName")%></td>
	    	<td class=tablebold>:</td><td><%=studentname %></td></tr>
	    	<tr><td class=tablebold><%=bundle1.getString("label.feespaidlistdetails.Standard")%></td>
	    	<td class=tablebold>:</td><td><%=studentstandardname%></td>
	    	<td class=tablebold><%=bundle1.getString("label.feespaidlistdetails.Section")%></td>
	    	<td class=tablebold>:</td><td><%=studentsectionname%></td></tr>
	    	<tr><td class=tablebold><%=bundle1.getString("label.feespaidlistdetails.Date")%></td>
	    	<td class=tablebold>:</td><td colspan=4><%=disfeesdate%></td></tr>
	    	
	    	<%
		    }else
		    	{
		    	out.println("<tr align='center' class='tablebold'><td colspan='4'><font color='red'>Data Not Found</font></td></tr>");
		    	//out.println("<tr><td colspan=4>Data not Found</td></tr>");
		    	}%>	 
	       </table>
	    </td>
	    </tr>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="10"><%=bundle1.getString("label.feespaidlistdetails.SL.No")%></th>
        	<th scope="col" class="rounded"> <%=bundle1.getString("label.feespaidlistdetails.BankName")%></th>
        	<th scope="col" class="rounded"> <%=bundle1.getString("label.feespaidlistdetails.DDNumber")%> </th>
        	<th scope="col" class="rounded"> <%=bundle1.getString("label.feespaidlistdetails.DDDate")%> </th>
        	<th scope="col" class="rounded"> <%=bundle1.getString("label.feespaidlistdetails.Amount")%></th>
       <%--  	<th scope="col" class="rounded"> <%=bundle1.getString("label.feespaidlistdetails.PaymentType")%></th> --%>
        	<th scope="col" class="rounded"> <%=bundle1.getString("label.feespaidlistdetails.CollectionDate")%></th>
            <th scope="col" class="rounded-q4"> <%=bundle1.getString("label.feespaidlistdetails.PaymentType1")%></th>
    ...
    <tfoot>
     ...
     <tbody>
   	    	...
   	    	splitdate=date.substring(0,10);
      	    studentfeescollectionList = feesscheduleQuery.studentFeescollectionDetailsforDate(studentapprovalid,splitdate);
	    	...
	    	for(int y=0;y<studentfeescollectionList.size();y++){
	    		...
	    		String feescollectiondate      = (String)studentfeescollectionListMap.get("feescollectdate");
		...
		<td><%= paymenttype%></td>
		  ...
		  <a href="#" class="bt_blue" onclick=backfun()><span class="bt_blue_lft"></span><strong><%=bundle1.getString("button.feespaidlistdetails.Back")%></strong><span class="bt_blue_r"></span></a>
      	      <a href="#" class="bt_green" onclick="printfun()"><span class="bt_green_lft"></span><strong><%=bundle1.getString("button.feespaidlistdetails.PrintPreview")%></strong><span class="bt_green_r"></span></a>
      
 <%}
    else
    {
    	out.println("Data Not Found");
        out.print("<a href='./FeesCollection_pre.jsp' class='bt_blue'><span class='bt_blue_lft'></span><strong>"+bundle1.getString("button.feespaidlistdetails.Back")+"</strong><span class='bt_blue_r'></span></a>");
    }
    %>
	<input type='hidden' name='radioname' value=''/>
	<input type='hidden' name='feedate' value='<%=date %>'/>
	<input type='hidden' name='disfeesdate' value='<%=disfeesdate %>'/>
