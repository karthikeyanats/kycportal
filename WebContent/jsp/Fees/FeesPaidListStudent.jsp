...
{
	window.open("PrintPreviewFeesdetails.jsp?rollno="+document.feescollectionform.reqrollnumbertext.value+"&feescollectionid="+obj.getAttribute("feescollectionid")+"","Msg","top=250,left=350,help:no,center:no,status:no,width=435,height=480,scrollbars=1;")
}
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
        	...
        	<th scope="col" class="rounded"><%=bundle1.getString("label.feespaidlist.SL.No")%></th>
        	<th scope="col" class="rounded"><%=bundle1.getString("label.feespaidlist.Date")%></th>
        	<th scope="col" class="rounded"><%=bundle1.getString("label.feespaidlist.FeesDetails")%></th>
    ...
    <tfoot>
     ...
     <tbody>
	    	...
	    	for(int y=0;y<studentfeescollectionList.size();y++){
	    		...
	    		String feescollectdate      = (String)studentfeescollectionListMap.get("feescollectdate");
		...
		<tr>
		<%
		if(!temp_termid.equals(feescollectdate))
		{
		out.println("<tr><td></td><td>"+inc+"</td><td  class=tablebold>"+feescollectdate+"</td>");
		out.println("<td  class=tablebold><a href ='#' feescollectiondate='"+feescollectiondate+"'  feescollectdate='"+feescollectdate+"' onclick=feesdetails(this) >Fees Details</a></td><td></td></tr>");

		temp_termid = feescollectdate;
		inc++;
		}
		} }else
		    	{
