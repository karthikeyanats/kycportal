...
<title>Know Your Child - Pending Fees Select | Powered by i-Grandee</title>
...
<script type="text/javascript">
	...
	function resetBoard()
	{
		...
		document.ApplicantSearch.term.value = "00";
	}
	...
	function viewStudent()
	{
		...
		document.ApplicantSearch.req_termfeesid.value  = document.ApplicantSearch.term.options[document.ApplicantSearch.term.options.selectedIndex].value;
		
		var termfeesIdAmount = document.ApplicantSearch.req_termfeesid.value;
		...
		else if(document.ApplicantSearch.req_termfeesid.value=="00")
		{
			alert("Select Term")
			document.ApplicantSearch.term.focus();
		...
		else
		{
			...
			document.ApplicantSearch.req_termfeesid.value 		=  termfeesIdAmount.split("@")[0];
			document.ApplicantSearch.req_termfeesamount.value 	=  termfeesIdAmount.split("@")[1];
			//document.ApplicantSearch.req_termfeesid.value 	= document.ApplicantSearch.term.options[document.ApplicantSearch.term.options.selectedIndex].value;
			document.ApplicantSearch.req_termfeesname.value = document.ApplicantSearch.term.options[document.ApplicantSearch.term.options.selectedIndex].text;
	
			document.ApplicantSearch.action="FeesPendingView.jsp";
			document.ApplicantSearch.submit();
		}
	...
	<table id="rounded-corner" border=0 summary="Department List" width="100%">
    	<thead>
    		<tr>
            	...
            	<th scope="col" class="rounded" width="100%">Pending List</th>
		...
		<%
			...
			else if(request.getParameter("req_sessionid") != null && request.getParameter("req_boardid") != null && request.getParameter("reg_coursepubid_hidden") != null )
			{
				...
				termList	=	feesscheduleQuery.loadTermFeesDetails(standardpublishid);
			...
			<tr>
				...
				<td><select name=section class=tablelight >
					...
					%>
						...
						<tr>
				<td class=tablebold>Term <font color="red">*</font></td>
				<td><select name=term class=tablelight >
					<option value="00">-Select Term-</option>
					<%
					...
					if(termList != null && termList.size()>0){
						...
						termFeesAmount  = (String)termhashmap.get("termfeesamount");
						out.println("<option value='"+termFeesId+"@"+termFeesAmount+"'>"+termName+"</option>");
					}
					}
    			...
    			<a href="#" class="bt_blue" onclick="viewStudent()"><span class="bt_blue_lft"></span><strong>View Fees Pending Students</strong><span class="bt_blue_r"></span></a>
