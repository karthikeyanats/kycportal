...
if(action=="A")   
{
if(confirm("Do you Want To Activate the Fees Head...?"))
{
...
<%@ page import="java.util.ArrayList,java.util.HashMap,com.iGrandee.MasterEntries.Standard.StandardQuery"%>
     ...
     try
     {
    	 instituteid = (String)session.getValue("instituteid");
    	  feesheadList = feesheadQuery.listFeesHead(instituteid,"D");
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="5%"><%=bundle.getString("label.feesheaddeactivelist.SL.No")%></th>
            <th scope="col" class="rounded" width="42%"><%=bundle.getString("label.feesheaddeactivelist.FeesHeadName")%></th>
            <th scope="col" class="rounded" width="20%"><%=bundle.getString("label.feesheaddeactivelist.Dateofcreation")%></th>
              <th scope="col" class="rounded-q4" colspan="1" width="30%"><%=bundle.getString("label.feesheaddeactivelist.Action")%></th>
    	 ...
    	 if(feesheadList!=null && feesheadList.size()>0)
			{
		 	for (int i = 0, j = 1; i < feesheadList.size(); i++) {
				...
				<td><a href="#" feesheadid="<%=feesheadid%>" feesheadname="<%=feesheadname%>" onclick="ActionFun(this,'A')"><%=bundle.getString("hlink.feesheaddeactivelist.Activate")%></a></td>
...
<input type="hidden" name="feesheadid_temp" value="">
	...
	<a href="#" onclick="backfun()" class="bt_blue"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.feesheaddeactivelist.BackToFeesHead")%> </strong><span class="bt_blue_r"></span></a>
