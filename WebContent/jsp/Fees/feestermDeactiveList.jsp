...
<title>Know Your Child - <%=bundle.getString("label.feestermdeactivelist.title")%> | Powered by i-Grandee</title>
...
if(action=="A")   
{
if(confirm("Do you want to Active Fees Term..?"))
{
document.standardForm.action="FeestermAction.jsp?mode_of_operation=activeststus";
	document.standardForm.submit(); 
}
}
}
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2>Fees / Fees Term / Fees Term Deactive List</h2>
...
<%@ page import="java.util.ArrayList,java.util.HashMap"%>
     ...
     try
     {
    	 instituteid = (String)session.getValue("instituteid");
    	  feestermList = feestermQuery.listFeesTerm(instituteid,"D");
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="1%"><%=bundle.getString("label.feestermdeactivelist.SL.No")%></th>
            <th scope="col" class="rounded" width="42%"><%=bundle.getString("label.feestermdeactivelist.FeesTermName")%></th>
            <th scope="col" class="rounded" width="15%"><%=bundle.getString("label.feestermdeactivelist.Dateofcreation")%></th>
              <th scope="col" class="rounded-q4" width="35%" colspan=1><%=bundle.getString("label.feestermdeactivelist.Action")%></th>
        </tr>
    </thead>
    <tfoot>
    	<tr>
    	 ...
    	 if(feestermList!=null && feestermList.size()>0)
			{
		 	for (int i = 0, j = 1; i < feestermList.size(); i++) {
				...
				<td><a href="#" termid="<%=termid %>" termname="<%=termname %>" onclick="ActionFun(this,'A')" ><%=bundle.getString("hlink.feestermdeactivelist.Active")%></a></td>
				</tr>
				<%
		 }
			}
...
<a href="#" onclick="backfun()" class="bt_blue"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.feestermdeactivelist.BackToFeesTerm")%></strong><span class="bt_blue_r"></span></a>
