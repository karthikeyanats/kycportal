...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle1.getString("label.feespaiddetails.rootpath")%></h2>
...
<tr>
	<td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.AcademicYear")%></td><td class=tablebold>:</td><td class=tablelight ><%=yearofsession %></td>
	<td class=tablebold><%=bundle1.getString("label.feespaiddetails.Board")%> </td><td class=tablebold>:</td><td  colspan=2  class=tablelight><%=boardName %></td>
</tr>
<tr><td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.Standard")%></td><td class=tablebold>:</td><td class=tablelight><%=request.getParameter("standardnamegp") %></td>
<td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.Term")%></td><td class=tablebold>:</td><td class=tablelight><%=request.getParameter("req_termfeesname")%></td>
</tr>
<tr><td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.DueDate")%></td><td class=tablebold>:</td><td class=tablelight><%=request.getParameter("req_duedate")%></td>
<td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.TermFeesAmount")%></td><td class=tablebold>:</td><td class=tablelight><%=formattedTotTermFeeCurrency %></td>
</tr>
</table>
</td></tr>
</table>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            ...
            <!-- <th scope="col" class="rounded" width="12%">Scholarship</th> -->
            <th scope="col" class="rounded-q4" width="12%"><%=bundle1.getString("label.feespaiddetails.PaidAmount")%></th>
            <!--  <th scope="col" class="rounded-q4" width="15%"><%=bundle1.getString("label.feespaiddetails.PendingAmount")%></th>-->                       
        </tr>
    </thead>
    <tfoot>
    	<tr>
	...
	String formattedRemAmountCurrency1 ="";
	...
	if(pendingFeesList.size()!=0)
	{
		...
		//	scholarship				= 	(String) map.get("scholarshipamount");
		  ...
		  /*   Double remAmountObj1 = Double.valueOf(scholarship);
		    NumberFormat pendingFormat1 = NumberFormat.getCurrencyInstance(localcurrency);
		    formattedRemAmountCurrency1 = pendingFormat1.format(remAmountObj1); */

			
			
			if(pendingstatus.equals("0")){
				count=count+1;	
				out.print("<tr><td>"+(count)+"</td><td>"+applicantName+"</td><td>"+sectionName+"</td><td>"+rollNo+"</td><td>"+formattedFeesPaidCurrency+"</td></tr>");
			}
		}
	}
	else
