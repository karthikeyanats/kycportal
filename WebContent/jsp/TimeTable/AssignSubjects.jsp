...
<script language='javascript'>
	...
	function loadStaffs()
	{
		if(check_ForParticularElements(document.workallotment.board,"select",document.workallotment.standard,"select",document.workallotment.section,"select")){
	...
	function changeSession(){
		...
		if(check_ForParticularElements(document.workallotment.board,"select",document.workallotment.standard,"select")){
			document.workallotment.section.options.selectedIndex	= 0;
			document.workallotment.submit();
		}
	}
	...
	function submitWorks()
	{
		...
		if(count >0){
			...
			if(status){
				...
				document.workallotment.action = "WorkAssignmentSubmit.jsp";
				document.workallotment.submit();
			}
		}
		else{
	...
	function checkTotal(obj){
		...
		if(diff>parseInt(parseInt(document.workallotment.remaining.value,10))){
			...
			alert("Allotted hours must be less than or equal to the Total hours.\n Please check the remaining hours.");
			document.workallotment.hours[i].value = document.workallotment.hours[i].getAttribute("oldvalue");
			document.workallotment.hours[i].focus();
			return false;
		}		
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.schedule.assignsubjects")%></h2>
...
<%
	...
	if(sectionid != null && !sectionid.equals("") && !sectionid.equalsIgnoreCase("null") && !sectionid.equalsIgnoreCase("0")){
		subjectList			= wallot.loadAllSubjects(staffid,sectionid); 
	}
...
%>
	<form name='workallotment' method='post'>
		...
		<table id="rounded-corner" border=0 summary="Department List" width="100%">
		    <thead>
		    	<tr>
		            ...
		            <th scope="col" class="rounded" width="35%"><%=bundle.getString("label.schedule.mode")%></th>
			...
			<%
				if(subjectList != null && subjectList.size()>0){
					for(int y=0;y<subjectList.size();y++){
						...
						if("Allotted".equals((String)hashmap.get("status"))){
							out.print("<tr><td class='tablelight'>"+(y+1)+"</td><td class='tablelight'>"+hashmap.get("subjectname")+"</td><td class='tablelight'>"+hashmap.get("subjectypename")+"</td><td class='tablelight'><input type='text' name='hours' size='4' maxlength='3' value='"+hashmap.get("allottedhours")+"' disabled ></td><td class='tablelight'><input type='checkbox' name='schedule' value='"+hashmap.get("subjectscheduleid")+"' disabled></td></tr>");
						}
						else{
							out.print("<tr><td class='tablelight'>"+(y+1)+"</td><td class='tablelight'>"+hashmap.get("subjectname")+"</td><td class='tablelight'>"+hashmap.get("subjectypename")+"</td><td class='tablelight'><input type='text' name='hours' size='4' maxlength='3' value='"+hashmap.get("allottedhours")+"' pos='"+y+"' oldvalue='"+hashmap.get("allottedhours")+"' onblur='checkTotal(this)'></td><td class='tablelight'><input type='checkbox' name='schedule' value='"+hashmap.get("subjectscheduleid")+"' onclick='countChecked(this)'></td></tr>");
						}
					}
				}
				else{
		...
		<%										
			if(subjectList != null && subjectList.size()>0){
		%>
			<a href="#" id="savebut"  class="bt_green" onclick='submitWorks()'><span class="bt_green_lft"></span><strong><%=bundle.getString("label.schedule.assign")%></strong><span class="bt_green_r"></span></a>
		<%
			} 
		%>
