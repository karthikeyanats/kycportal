...
function viewdepartment(obj)
{
	//if(confirm("Do You Want to Edit the Staff...?" ))
	{
		...
		document.f1.action= "./StaffProfileView.jsp";
		document.f1.submit();
	}
}
function backfun()
{
document.f1.action= "./Staff_List.jsp?instituteid="+document.f1.req_instituteid.value;
document.f1.submit();
}
...
%>
    ...
    <h2><%=staffquery.fieldValue("institutename","institution","instituteid",request.getParameter("req_instituteid"),"status",request.getParameter("req_instituteid"))%> / <%=bundle.getString("label.superadmin.stafflistrootpath") %> / <%=request.getParameter("deptname")%> </h2>
    
<form name="f1" method="post" action="" >
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="10%"><%=bundle.getString("label.stafflistview.SL.No") %></th>
            ...
            <th scope="col" class="rounded-q4" width="50%"><%=bundle.getString("label.stafflistview.JobCategory") %></th>
