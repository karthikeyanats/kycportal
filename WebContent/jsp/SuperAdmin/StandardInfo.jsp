...
{
	document.superadminForm.standardscheduleid.value=obj.getAttribute("standardscheduleid");
	document.superadminForm.standardgroup.value=obj.getAttribute("standardgroup");
...
<form name='superadminForm' method='post'>
 <jsp:useBean id="instituteQuery" scope="page"
	...
	standardList = instituteQuery.listStandard(sessionid,boardid,instituteid); 
	%>
	...
	<input type="hidden" name="standardgroup" value="">
	
		<br><br>
	<h2><%=bundle.getString("label.superadmin.standardrootpath")%></h2>
	<tr>
	<td>
	...
	<table id="rounded-corner" border=0  width="100%">
	    <thead>
	    	<tr>
	            ...
	            <th scope="col" class="rounded" width="40%"><%=bundle.getString("label.usernotutil.standardname")%></th>                                   
	            <th scope="col" class="rounded-q4" ><%=bundle.getString("label.superadmin.noofexams")%></th>
	        </tr>
	    </thead>
	    <tfoot>
	    	<tr>
			...
			if(standardList!=null && standardList.size()>0)
			{
				...
				{//b.standardname,a.standardscheduleid,groupname
					HashMap standardListMap=(HashMap)standardList.get(i);
					String standardscheduleid=standardListMap.get("standardscheduleid")+"";
					String groupname=standardListMap.get("groupname")+"";
					String standardgroup="";
					if(groupname.equals("NoGroup"))
					{
						standardgroup=standardListMap.get("standardname")+"";
					}
					else
					{
						standardgroup=standardListMap.get("standardname")+" "+groupname;

					}
					ArrayList examcount=instituteQuery.examCountStandard(instituteid,sessionid,standardscheduleid);
					out.print("<tr valigh='top' class='tablelight'>");
					out.print("<td>"+(i+1)+"</td>");
					out.print("<td>"+standardgroup+"</td>");
					if(examcount.size()>0)
					{
						out.print("<td><a href='#' standardscheduleid='"+standardscheduleid+"' standardgroup='"+standardgroup+"' onclick='clickFun(this)'>"+examcount.size()+"</td>");

					}
					else
					{
						out.print("<td >"+examcount.size()+"</td>");
