...
<title>Know Your Child - <%=bundle.getString("label.photogallery.entrytitle") %>  | Powered by i-Grandee</title>
...
{   
	...
	document.curricularevententryform.curriculareventname.value=obj.getAttribute("curriculareventname");
	document.curricularevententryform.venue.value=obj.getAttribute("venue");
	document.curricularevententryform.eventdate.value=obj.getAttribute("eventdate");
...
document.curricularevententryform.sessionstatus_temp.value=obj.getAttribute("sessionstatus_temp");
document.curricularevententryform.venue.value=obj.getAttribute("venue");
document.curricularevententryform.eventdate.value=obj.getAttribute("eventdate");
document.curricularevententryform.curriculareventname.value=obj.getAttribute("curriculareventname");
document.curricularevententryform.curriculareventid.value=obj.getAttribute("curriculareventid");
document.curricularevententryform.action="phodetails.jsp";
document.curricularevententryform.submit();

}
function trashlist(obj)
{
document.curricularevententryform.venue.value=obj.getAttribute("venue");
document.curricularevententryform.eventdate.value=obj.getAttribute("eventdate");
document.curricularevententryform.sessionstatus_temp.value=obj.getAttribute("sessionstatus_temp");
document.curricularevententryform.curriculareventname.value=obj.getAttribute("curriculareventname");
document.curricularevententryform.curriculareventid.value=obj.getAttribute("curriculareventid");
document.curricularevententryform.action="phototrashlist.jsp";
document.curricularevententryform.submit();
...
String sessionstatus_temp=request.getParameter("sessionstatus")+"";
String curricularcategory=request.getParameter("curricularcategory")+"";
//out.println(yearofsession+curricularcategory+sessionstatus_temp);
...
<tr class=tablebold><td> <%=bundle.getString("label.photogallery.academicsession") %> : <select name="yearofsession" validate='Year Of Session' class="tablelight" onChange="loadyearofsession()">
 ...
 <%
  		 sessionList =  PhotoGalleryQuery.listYearofSession(instituteid);
        	if(sessionList!=null && sessionList.size()>0)  
			{
				...
				{
       		      out.println("<option value='"+sessionid+"' sessionstatus='"+sessionstatus+"' class=tablelight selected>"+sessionname+"</option>");
				}
				else
				{
		       		 out.println("<option value='"+sessionid+"' sessionstatus='"+sessionstatus+"' class=tablelight >"+sessionname+"</option>");
...
<table id="rounded-corner" border=0 summary="selectd Information" align=center>
    <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="10%"><%=bundle.getString("label.photogallery.slno") %></th>
        	<th scope="col" class="rounded" width="30%" ><%=bundle.getString("label.photogallery.eventname") %></th>
        	<th scope="col" class="rounded" width="20%"><%=bundle.getString("label.photogallery.venue") %></th>
        	<th scope="col" class="rounded" width="15%"><%=bundle.getString("label.photogallery.eventdate") %></th>
        	<th scope="col" class="rounded-q4" width="25%" colspan=2><%=bundle.getString("label.photogallery.action") %></th>
        </tr>
    </thead>
    <tfoot>
    	<tr>
	...
	if(eventList!=null && eventList.size()>0)
	{
		...
		if(sessionstatus_temp.equals("A"))
		{
		out.println("<td colspan=2><a href=\"#\" curriculareventid='"+curriculareventid+"' curriculareventname='"+curriculareventname+"' venue='"+venue+"' eventdate='"+eventdate+"' reqaction='upload' onclick=gotoaction(this)>"+bundle.getString("hlink.photogallery.upload")+"</a>");
		}
		else if(sessionstatus_temp.equals("R"))
		{
			out.println("<td colspan='2'>Archival"); 

		}
		out.println("| &nbsp;&nbsp;<a href=\"#\" curriculareventid='"+curriculareventid+"' curriculareventname='"+curriculareventname+"' venue='"+venue+"'eventdate='"+eventdate+"' sessionstatus_temp='"+sessionstatus_temp+"' reqaction='view'  onclick=viewevents(this)>"+bundle.getString("hlink.photogallery.view")+"</a>");
		
		out.println("| &nbsp;&nbsp;<a href=\"#\" curriculareventid='"+curriculareventid+"' curriculareventname='"+curriculareventname+"' venue='"+venue+"' eventdate='"+eventdate+"' sessionstatus_temp='"+sessionstatus_temp+"' reqaction='trash' onclick=trashlist(this)>"+bundle.getString("hlink.photogallery.trashlist")+"</td></a>");
	...
	%>
		...
		<%
 if(sessionstatus_temp != null && sessionstatus_temp.equals("R"))
				{
					out.println("<div class='warning_box'>");
					out.println(bundle.getString("label.photogallery.trashpreviewerrormessagedetails"));
					out.println("</div>");		
				}%>
