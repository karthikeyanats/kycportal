...
<script type="text/javascript">
	...
	function genRollNo()
	{
		...
		else if(document.ApplicantSearch.rollnoprefix.value=="Set Roll No. Prefix")
		{
			alert("Set Roll No Prefix First")			
			//document.ApplicantSearch.rollnoprefix.focus();
	...
	function viewStudent()
	{
		...
		else
		{
			...
			document.ApplicantSearch.action="RollNoGenerateView.jsp";
			document.ApplicantSearch.submit();
		}
	
	}
	
	function setRollNoPrefix()
	{
		document.ApplicantSearch.req_sessionid.value  = document.ApplicantSearch.sessionname.options[document.ApplicantSearch.sessionname.options.selectedIndex].value;
		document.ApplicantSearch.req_sessionstatus.value = document.ApplicantSearch.sessionname.options[document.ApplicantSearch.sessionname.options.selectedIndex].getAttribute("archivestatus");
		document.ApplicantSearch.req_boardid.value  = document.ApplicantSearch.boardname.options[document.ApplicantSearch.boardname.options.selectedIndex].value;
		...
		else
		{
			...
			document.ApplicantSearch.standardnamegp.value  	= document.ApplicantSearch.course.options[document.ApplicantSearch.course.options.selectedIndex].text;
			document.ApplicantSearch.req_sectionname.value  = document.ApplicantSearch.section.options[document.ApplicantSearch.section.options.selectedIndex].text;
			if(document.ApplicantSearch.rollnoprefix.value == "Set Roll No. Prefix")
				document.ApplicantSearch.rollnoprefix.value = "";
			document.ApplicantSearch.action="RollNoPrefixSet.jsp";
			document.ApplicantSearch.submit();
		}
	
	}
	
function editRollNoPrefix()
	{
		document.ApplicantSearch.req_sessionid.value  = document.ApplicantSearch.sessionname.options[document.ApplicantSearch.sessionname.options.selectedIndex].value;
		document.ApplicantSearch.req_sessionstatus.value = document.ApplicantSearch.sessionname.options[document.ApplicantSearch.sessionname.options.selectedIndex].getAttribute("archivestatus");
		document.ApplicantSearch.req_boardid.value  = document.ApplicantSearch.boardname.options[document.ApplicantSearch.boardname.options.selectedIndex].value;
		...
		if(document.ApplicantSearch.rollnoprefix.value == "Set Roll No. Prefix")
		{
			alert("Roll No. Prefix is not yet Set");
			document.ApplicantSearch.section.focus();
			return;	
		}
...
<%@ include file="../include/header.jsp" %>
	...
	<table id="rounded-corner" border=0 summary="Department List" width="100%">
    	<thead>
    		<tr>
            	<th scope="col" class="rounded-company" width="80%">Roll No. Generation</th>
		...
		<%
			...
			else if(request.getParameter("req_sessionid") != null && request.getParameter("req_boardid") != null && (request.getParameter("reg_coursepubid_hidden") != null && request.getParameter("reg_coursepubid_hidden") != "") && (request.getParameter("req_sectionpubid") != null && request.getParameter("req_sectionpubid") != ""))
			{
				//System.out.println("ELSEA--->"+request.getParameter("req_sessionid")+"---"+request.getParameter("req_boardid")+"---"+request.getParameter("reg_coursepubid_hidden")+"---"+request.getParameter("req_sectionpubid"));
				String standardpublishid = request.getParameter("reg_coursepubid_hidden"); 
			//	out.println("standardpublishid===>>>"+standardpublishid);
				...
				rollnoPrefix = registrationQuery.loadRollNoPrefix(request.getParameter("req_sectionpubid"));
				if(rollnoPrefix.equals("null")||rollnoPrefix==null)
					rollnoPrefix = "Set Roll No. Prefix";
				
				//System.out.println("rollnoPrefix-->"+rollnoPrefix); 
			}
			...
			//out.print(request.getParameter("req_sessionstatus"));
		%>
			...
			<tr>
				...
				<td class=tablebold width='2%'>:</td>
				<td><select name=section class=tablelight onchange=getRollNoPrefix()>
			...
			<% 
				if(rollnoPrefix==null || rollnoPrefix.equals("null") || rollnoPrefix.trim().equals(""))
					rollnoPrefix = "-";
				//if(rollnoPrefix.equals("null"))
					//rollnoPrefix = "Assign";
					
			%>

			<tr>
				<td class=tablebold>Roll Number Prefix </td>
				<td class=tablebold width='2%'>:</td>
				<% 
				if(rollnoPrefix.equals("-")||rollnoPrefix=="-")
				{
				%>				
				<td><%=rollnoPrefix%></a><input type=hidden name=rollnoprefix value="<%=rollnoPrefix%>" readonly></td>
				<%
				}
				else if(rollnoPrefix.equals("Set Roll No. Prefix")||rollnoPrefix=="Set Roll No. Prefix")
				{
				%>
				<%
				...
				{
				%>
				<td><a href="#" onclick="setRollNoPrefix()"><%=rollnoPrefix%></a><input type=hidden name=rollnoprefix value="<%=rollnoPrefix%>" readonly></td>				
				<% 
				}
				else if(request.getParameter("req_sessionstatus") != null && request.getParameter("req_sessionstatus").equals("R"))
				{
					%>
					<td>-</td>				
					<% 
			...
			if(request.getParameter("req_sessionstatus") != null && request.getParameter("req_sessionstatus").equals("R"))
				{
				out.print("<tr><td width='43%' valign=top class='tablebold'>Archival Details</td><td class=tablebold width='2%' valign=top>:</td><td valign=top class='tablelight' ><h2>You can't Edit or Delete in an Archived Year</h2></td></tr>");	
				}
			%>							
			</table></td></tr>
			</table>
				...
				{
				%>    			
    			<a href="#" class="bt_green" onclick="genRollNo()"><span class="bt_green_lft"></span><strong>Generate Roll No</strong><span class="bt_green_r"></span></a>
    			<a href="#" class="bt_blue" onclick="editRollNoPrefix()"><span class="bt_blue_lft"></span><strong>Edit Roll No Prefix</strong><span class="bt_blue_r"></span></a>
    			<% 
				}
    			%>
