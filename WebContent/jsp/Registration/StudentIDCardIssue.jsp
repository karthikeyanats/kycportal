...
function loadstudent()
{
	if(check_ForParticularElements(document.StudentReport_Form.session,"select",document.StudentReport_Form.board,"select",document.StudentReport_Form.standardscheduleid,"select",document.StudentReport_Form.sectionscheduleid,"select"))
	{
		...
		/*document.StudentReport_Form.action="./StudentIDCardIssuePrintPreview.jsp"
		document.StudentReport_Form.submit();*/
		var str="";
		str="req_sessionname="+document.StudentReport_Form.req_sessionname.value+
			"&session="+document.StudentReport_Form.session.value+
			"&board="+document.StudentReport_Form.board.value+
			"&req_boardname="+document.StudentReport_Form.req_boardname.value+
			"&req_standardnamegp="+document.StudentReport_Form.req_standardnamegp.value+
			"&req_sessionstatus="+document.StudentReport_Form.req_sessionstatus.value+
			"&standardscheduleid="+document.StudentReport_Form.standardscheduleid.value+
			"&req_sectionname="+document.StudentReport_Form.req_sectionname.value+
			"&sectionscheduleid="+document.StudentReport_Form.sectionscheduleid.value;

		window.open("StudentIDCardIssuePrintPreview.jsp?"+str,"Msg","top=250,left=350,help:no,center:no,status:no,width=780,scrollbars=1;")
...
%>
		...
		<a href="#" class="bt_green" onClick='loadstudent()'><span class="bt_green_lft"></span><strong>ID Card Print Preview</strong><span class="bt_green_r"></span></a>
		</form>		
