...
<script language="JavaScript">
	...
	function submitfun1(){
		...
		document.MainForm.action= "./BulkProfileSubmitAut.jsp";
	...
	function submitfun(){
		//alert("Call submitfun");
		 if(document.MainForm.community.length){

			for(var t=0;t<document.MainForm.community.length-1;t++){
				
				if(!check_ForParticularElements(document.MainForm.community[t],"select")){
					//document.MainForm.community[t].focus();
					return  false;
				}
			}
		}
		 
		 if(document.MainForm.religion.length){

				for(var t1=0;t1<document.MainForm.religion.length-1;t1++){
					
					if(!check_ForParticularElements(document.MainForm.religion[t1],"select")){
						//document.MainForm.community[t].focus();
						return  false;
					}
				}
		...
		/* if(document.MainForm.scholarshipname != 'undefined') {
			alert("value===>>>"+document.MainForm.scholarshipname.value);
	 */		
			 /*if(document.MainForm.scholarshipname.length){
	
				for(var t=0;t<document.MainForm.scholarshipname.length-1;t++){
							
					if(!check_ForParticularElements(document.MainForm.scholarshipname[t],"select")){
						document.MainForm.scholarshipname.focus();
						return  false;
					}
				}
			}  */
	...
	function backfun()
	{
		document.MainForm.action= "./BulkProfileEntryAut.jsp";
		document.MainForm.submit();			
	}
...
<%@ include file="../include/header.jsp" %>
	...
	<form name="MainForm" method="post" action="" >
		...
		<%
	    	...
	    	String sessionid			= upload.getParameter("sessionid");
	    	String boardid				= upload.getParameter("boardid");
	    	String standardname			= upload.getParameter("standardname");
	    	String termname 			= upload.getParameter("termname");
	    	String standardid			= upload.getParameter("standard");
	    	String termfeesid			= upload.getParameter("term");
	    	String filepath					= upload.getFileNames()[0];
	    		    	
	    	ArrayList communityList		= qry.loadcommunity((String)session.getAttribute("instituteid"));
	    	ArrayList religionList		= qry.loadReligion((String)session.getAttribute("instituteid"));
	    	
	    	
	    	ArrayList scholarshipList   = qry.loadscholarshipname(termfeesid);
	    	
	    	HashMap hasMap				= null;			
	    	HashMap hasMap1				= null;
	    %>
		    	...
		    	<td colspan=2>
		     		<table  border=0 summary="Department List" width="100%">
						...
						<tr>
							...
							<td class=tablebold>:</td>
							<td><%=standardname%></td>
							<td class="tablebold" align=right>Term Name</td>
							<td class=tablebold>:</td>
							<td><%=termname%></td>
						</tr>
					</table>
	...
	<br> 
			...
			<table id="rounded-corner" border=0 summary="Department List" width="100%">
			    <thead>
			    	<tr>
			        	...
			        	<th scope="col" class="rounded">Guardian Mail</th>
			        	...
			        	<th scope="col" class="rounded">Community</th>
			        	<th scope="col" class="rounded">Religion</th>
			        	<th scope="col" class="rounded">Existing Student Roll No</th>
			        	<th scope="col" class="rounded-q4">Scholarship Name</th>
			        </tr>
			    </thead>
			    <%
					...
					HSSFWorkbook workBook 			= ExcelParser.getWorkBook(new FileInputStream(storagepath+filepath));
							...
							if(cells == 35)
							{
								if(j>0)
								{
									...
									*/
									...
									if(gender != null && (gender.equalsIgnoreCase("Male") || gender.startsWith("M")))
										gender	= "M";
									else
										gender	= "F";
									...
									HSSFCell cell16 		= row.getCell((short)15);
									...
									HSSFCell cell18 		= row.getCell((short)17);
									...
									HSSFCell cell19 		= row.getCell((short)18);
									...
									HSSFCell cell22 		= row.getCell((short)21);
									...
									HSSFCell cell22a 		= row.getCell((short)22);
									guardianmail 			= (String)ExcelParser.getValueOfCell(cell22a,"CHAR");
									out.print("<td><input type='hidden' name='guardianmail' value='"+guardianmail+"'>"+guardianmail+"</td>");
									...
									community		= (String)ExcelParser.getValueOfCell(cell31,"CHAR");
									
									
									out.print("<td><select name='community' validate='Community'><option value='0'>-Select Community-</option>");
									
									if(communityList != null && communityList.size()>0)
									for(int y=0;y<communityList.size();y++){
										hasMap		= (HashMap)communityList.get(y);
																				
										if(community != null)
										{
											
											if(community.equalsIgnoreCase((String)hasMap.get("communityname"))){
												out.print("<option value='"+hasMap.get("communityid")+"' selected>"+hasMap.get("communityname")+"</option>");
												
												}
											else{
												out.print("<option value='"+hasMap.get("communityid")+"'>"+hasMap.get("communityname")+"</option>");
									...
									out.print("</select></td>");									
									System.out.print("cell32==");  

									HSSFCell cell32 	= row.getCell((short)32);
									religion		= (String)ExcelParser.getValueOfCell(cell32,"CHAR");
									
									
									out.print("<td><select name='religion' validate='Religion'><option value='0'>-Select Religion-</option>");
									
									if(religionList != null && religionList.size()>0)
									for(int y=0;y<religionList.size();y++){
										hasMap		= (HashMap)religionList.get(y);
																				
										if(religion != null)
										{
											
											if(religion.equalsIgnoreCase((String)hasMap.get("religionname"))){
												out.print("<option value='"+hasMap.get("religionid")+"' selected>"+hasMap.get("religionname")+"</option>");
												
												}
											else{
												out.print("<option value='"+hasMap.get("religionid")+"'>"+hasMap.get("religionname")+"</option>");
									...
									HSSFCell cell33 		= row.getCell((short)33);						   

									existingstudentrollno			= (String)ExcelParser.getValueOfCell(cell33,"CHAR");
									if(existingstudentrollno != null){
										out.print("<td><input type='hidden' name='existingstudentrollno' value='"+existingstudentrollno+"'>"+existingstudentrollno+"</td>");
									}
									else {									
										out.print("<td><input type='hidden' name='existingstudentrollno'>-</td>");
									}
																	
									
									
																	
								 	HSSFCell cell34 		= row.getCell((short)34); 
									scholarshipname			= (String)ExcelParser.getValueOfCell(cell34,"CHAR");
									  
									
									 out.print("<td>");
									 out.print("<select name='scholarshipname' validate='Scholarship Name'><option value='0'>-Select Scholarship Name-</option>");
									 if(scholarshipList != null && scholarshipList.size()>0){
										 
										
										 
											for(int y=0;y<scholarshipList.size();y++){
									 	hasMap1		= (HashMap)scholarshipList.get(y);
									 	
										
										if(scholarshipname != null)
										{
											if(scholarshipname.equalsIgnoreCase((String)hasMap1.get("scholarshipname")))
											{
												
												out.print("<option value='"+hasMap1.get("scholarshipamountallocationid")+"' selected>"+hasMap1.get("scholarshipname")+"</option>");
											}
											else{
												
												out.print("<option value='"+hasMap1.get("scholarshipamountallocationid")+"'>"+hasMap1.get("scholarshipname")+"</option>");
												//out.print("not equal");
												}
										}										
			    ...
			    <tfoot>
			    	<tr>
			        	<td  colspan='35' class="rounded-foot-left"><em></em></td>
			        	<td class="rounded-foot-right">&nbsp;</td>
			       </tr>
			    </tfoot>
			</table>
		...
		<% if(totalcount>0){ %> 
 		...
 		<input type='hidden' name='termfeesid' value='<%=termfeesid%>'>
 		...
 		<%
			...
			out.print("<input type='hidden' name='guardianmail' 			disabled>");			
			...
			out.print("<input type='hidden' name='community' 				disabled>");
			out.print("<input type='hidden' name='religion' 				disabled>");
			out.print("<input type='hidden' name='scholarshipname'			disabled>");
