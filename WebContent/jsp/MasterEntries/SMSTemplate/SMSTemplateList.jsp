...
function backfun()
{
		document.smstemplatelistform.action="SMSTemplate.jsp";
		document.smstemplatelistform.submit();
}
function updatefun(obj)
{
	if(obj.getAttribute("act") == "Trash")
	{
		if(confirm("do you want to trash this tempalte?"))
		{
		
			document.smstemplatelistform.smsoperation.value="UpdatStatus";
			document.smstemplatelistform.requpdatestatus.value="Trash";
	...
	{
		if(confirm("do you want to upadte this tempalte?"))
		{
		
			document.smstemplatelistform.smsoperation.value="UpdateStatus";
			document.smstemplatelistform.requpdatestatus.value="Trash";
			document.smstemplatelistform.reqtemplateid.value =obj.getAttribute("templateid");
			document.smstemplatelistform.reqtemplatename.value =obj.getAttribute("templatename");
			document.smstemplatelistform.reqtemplatedescription.value =obj.getAttribute("templatedescription");
			document.smstemplatelistform.action="SMSTemplateUpdate.jsp";
			document.smstemplatelistform.submit();
		}
	}
}
...
<%@ include file="../../include/header.jsp" %>
    ...
    <h2>General/ SMS Template / SMS Template List</h2>
<form name="smstemplatelistform" action="" method="post">
  <%@ page import="java.util.ArrayList,java.util.HashMap,com.iGrandee.MasterEntries.SMSTemplate.SMSTemplateQuery"%>
   ...
   templateList = smstemplateQuery.listSMSTemplateList(instituteid,"Active");
   try{
   int inc = 1;
...
out.print("<tr><td>"+inc+"</td><td>"+smstemplatename+"</td><td>"+smsdescription+"</td><td>"+dateofcreation+"</td><td><a href='#' templateid='"+smstemplateid+"' templatename='"+smstemplatename+"' templatedescription='"+smsdescription+"' act='Update' onclick=updatefun(this)>Update</a> | <a href='#' act='Trash' templateid='"+smstemplateid+"' onclick=updatefun(this)>Trash</a></td></tr>");	    			
		inc++;			    		
		}
    }
    else
   ...
   <input type=hidden name="reqtemplatename" value=""/>
   <input type=hidden name="reqtemplatedescription" value=""/>
