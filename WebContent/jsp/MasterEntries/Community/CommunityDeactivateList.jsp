...
try{
	...
	String CommStatus = "Deactivated";
	communityBean.setCommstatus(CommStatus);
	
	result = community_query_object.CommunityDeactivate(communityBean);
	ArrayList checkdeactive=community_query_object.checkdeactiveCommunity(instituteid,communityid);
	//out.println(checkdeactive.size());
	if(result>0)
    op=" Community Deactivated Successfully";
	else
		op="Community Deactivation Failed";
	return_file = request.getContextPath()+"/jsp/MasterEntries/Community/CommunityDeactivatedViewList.jsp";
