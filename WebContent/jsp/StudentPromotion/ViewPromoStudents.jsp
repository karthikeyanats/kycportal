...
function allocatefun()
{
		...
		if(document.approvalForm.chk.length==undefined)
		{
			if(document.approvalForm.chk.checked==true)
			{
				...
				{			
					if(confirm("Do you want to Allocate the Promoted Student..?"))
					{
					document.approvalForm.action="./promotionAllocationSubmit.jsp";
  		...
  		if(test==true )
		{
			...
			{		
				if(confirm("Do you want to Allocate the Promoted Student(s)..?"))
				{
	    	   	document.approvalForm.action="./promotionAllocationSubmit.jsp";
	    	  	document.approvalForm.submit();
	    	  	}
...
function goBack()
{
	document.approvalForm.action="PromotedStudentAllocation.jsp";
	document.approvalForm.submit();
}
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2>Enrollment / Student Enrollment / Promoted Student Allocation</h2>
...
<table id="rounded-corner" border=0 width="100%">
    <thead>
	 ...
	 <%
		...
		registeredList	=	(ArrayList)promotionQuery.displayPromotedDetails(stdscheduleid);
		...
		if(registeredList.size()!=0)
		{
			...
			for(int i=0;i<registeredList.size();i++)
			{
				...
				allocationStatus	=	(String) map.get("allocationafterpromotion");
				
				if(allocationStatus.equals("NO"))
				{
					checkCount++;
...
<table id="rounded-corner" border=0 width="100%">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded-q4" width="20%">Section</th>            
        </tr>
    </thead>
    <tfoot>
    	<tr>
	...
	if(registeredList.size()!=0)
	{
		for(int i=0;i<registeredList.size();i++)
		{
			...
			dateofapproval 		=	(String) map.get("dateofallocation");
			promotionStatus		=	(String) map.get("promotionstatus");
			studentPromotionId	=	(String) map.get("studentpromotionid");
			allocationStatus	=	(String) map.get("allocationafterpromotion");
			allocatedSection	=	(String) map.get("sectionname");
			
			if(rollNo==null || rollNo.equals("null"))
				rollNo = "-";
			
			out.print("<tr><td>"+(i+1)+"</td><td>"+applicantName+"</td><td>"+applicationNo+"</td><td>"+rollNo+"</td>");
			if(allocationStatus.equals("NO"))
			{
				flag = true;
				out.println("<td align='left'><input type='checkbox' name='chk' value="+studentAllocationId+"@"+candidateregisterid+"@"+rollNo+"@"+studentPromotionId+"@"+promotionStatus+"@"+count+" onclick=chkval(this)></td><td>-</td></tr>");
			}
			else
			{
				...
				out.print("<td>-</td><td>"+allocatedSection+"</td></tr>");
