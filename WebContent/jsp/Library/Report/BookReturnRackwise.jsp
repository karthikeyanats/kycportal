...
<%@page language="java" import="java.util.ArrayList,java.util.HashMap,java.util.ResourceBundle"%>
<%
...
<html>
<head>
<title><%=bundle.getString("label.bookreturn.entrytitle") %></title>
...
{var status=false;
	...
	if(status)
	{
		document.MainForm.action= "BookReturnList.jsp";
...
function searchfun()
{
		document.MainForm.action= "BookReturnRackwise.jsp";
		document.MainForm.submit(); 
}
...
<jsp:useBean id="query_object" scope="page" class="com.iGrandee.Library.MasterEntries.Rack.RackEntry" />
<%
String date_str="";
...
<%@ include file="../../include/header.jsp" %>
    ...
    <div class="right_content">
         <h2><%=bundle.getString("label.bookreturn.entryrootpath")%> / Rackwise Search</h2>
...
<table id="rounded-corner" border=0 width="100%">
    <thead>
    	<tr>
        	...
        	<th scope="col" class="rounded-company" width="40%"><%=bundle.getString("label.bookreturn.entrytableheading") %></th>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    ...
    <thead>
    		<tr valign="top">
            <td class="tablebold" valign="top"><%=bundle.getString("label.bookreturn.StartDate") %>
            </td>
            <td class="tablebold" width="1%">:</td>
            <td>
            <input type=text name="startdate"  validate="Start Date"  onkeydown=this.blur()>&nbsp;&nbsp;
			<input type="button" class="tablehead"  value="^" onclick="document.MainForm.startdate.value='';document.MainForm.enddate.value='';Calendar_Fun('/kycportal/js/Calendar/CalendarWindow.html','MainForm.startdate','0',event,'Not_Greater_Than_CurrentDate',document.MainForm.server_date);">
			</td>					
			<td class="tablebold" valign="top"><%=bundle.getString("label.bookreturn.EndDate") %>
            </td>
            <td class="tablebold" width="1%">:</td>
             <td>
			...
			<tr height=30 >
					<td class=tablebold align=center width='100%' colspan=6>Rack&nbsp;&nbsp;:
					<select name="rackname" style="width:170px" class="tablelight">
							<option value="program"> -- Select Rack -- </option>
							<%
							String instituteid 	= session.getValue("instituteid")+"";
							ArrayList deptList		= query_object.get_All_Rack(instituteid,"A");							
								if(deptList!=null)
								{
									for(int i=0;i<deptList.size();i++){
										HashMap maps = (HashMap)deptList.get(i);
										String RACKID = (String)maps.get("rackid");
										String NAME = (String)maps.get("rackname");
							%>
...
<br>
	 ...
	 <a href="#" class="bt_blue" onclick="backfun()"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.bookreturn.Back")%></strong><span class="bt_blue_r"></span></a>
