...
{
 	window.open("./printRenew.jsp?startdate="+document.MainForm.startdate.value+"&enddate="+document.MainForm.enddate.value+"&flage_temp="+document.MainForm.flage_temp.value,"Msg","top=250,left=350,help:no,center:no,status:no,width=650,height=800,scrollbars=1;")
...
ArrayList JournalList=new ArrayList();
	...
	{
		BookList=query_object.get_All_BookRenewal(startdate,enddate,sessionid,"bookMaster",instituteid); 
	}
	else if(flage_temp.equals("Journal"))
	{
		JournalList=query_object.get_All_BookRenewal(startdate,enddate,sessionid,"Journal",instituteid);  
...
<%@ include file="../../include/header.jsp" %>
	...
	if(flage_temp.equals("Book"))
	{
    out.print("<h2>"+bundle.getString("label.bookrenewal.entryrootpath1")+"</h2>");
    
	}
	else if(flage_temp.equals("Journal"))
	{
	    out.print("<h2>"+bundle.getString("label.bookrenewal.entryrootpath2")+"</h2>");
   ...
   <table id="rounded-corner1" border=0 summary="Department List" width="100%">
    <thead>
    	<tr class=tableheading  >
        	<th scope="col" class="rounded-company" width="5%" rowspan=2><%=bundle.getString("label.bookrenewal.Sl.No") %></th>
            <th scope="col" class="rounded" width="10%"  rowspan=2 ><%=bundle.getString("label.bookrenewal.Title") %> </th>
            <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookrenewal.AccessNo") %></th>
             <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookrenewal.Author") %></th>
             <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookrenewal.RenewalDate") %></th>
            <th scope="col" class="rounded" width="10%" colspan=<%=3 %>>Renewed By </th>
            <tr  class=tableheading><th width="10%"><%=bundle.getString("label.bookrenewal.Username") %></th><th width="10%"><%=bundle.getString("label.bookrenewal.CardNo") %></th><th width="10%"><%=bundle.getString("label.bookrenewal.Designation") %></th>
		     </tr>
		    </thead>
      		<tfoot>
				...
				try
				{
			     if(BookList!=null && BookList.size()>0)
			     {
			    	 ...
			    	 for(int i=0,j=1;i<BookList.size();i++,j++)
			    	 {
			    	     ...
			    	     String renewaldate=BookListMap.get("renewaldate")+"";
			    		...
			    		if(renewaldate == null || renewaldate.equals("") || renewaldate.equals("-") || renewaldate.equalsIgnoreCase("null"))
						{
			    			out.print("<td class='tablelight' >-</td>");
						}
			    		else
			    		{   
			    		 out.print("<td class='tablelight' >"+BookListMap.get("renewaldate")+"</td>"); 
			    		}
...
%>
    ...
    <thead>
    	<tr class=tableheading  >
        	<th scope="col" class="rounded-company" width="5%" rowspan=2><%=bundle.getString("label.bookrenewal.Sl.No") %></th>
            <th scope="col" class="rounded" width="10%"  rowspan=2 ><%=bundle.getString("label.bookrenewal.JournalName") %> </th>
            <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookrenewal.CodeNo") %></th>
             <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookrenewal.BookType") %></th>
             <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookrenewal.RenewalDate") %></th>
            <th scope="col" class="rounded" width="10%" colspan=<%=3 %>>Renewed By </th>
            <tr  class=tableheading><th width="10%"><%=bundle.getString("label.bookrenewal.Username") %></th><th width="10%"><%=bundle.getString("label.bookrenewal.CardNo") %></th><th width="10%"><%=bundle.getString("label.bookrenewal.Designation") %></th>
		     </tr>
...
%>
		...
		<a href="BookRenewal.jsp" class="bt_blue" ><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.datewisebooksearch.Back") %> </strong><span class="bt_blue_r"></span></a>
