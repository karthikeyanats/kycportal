...
document.f1.action= "./CardIssueStaffList.jsp";
document.f1.submit();
}
function Backfun()
{
document.f1.action="./CardIssue.jsp";
document.f1.submit();
}
</script>
...
<%@ include file="../../include/header.jsp" %>  
    ...
    <h2><%=bundle.getString("label.cardissue.entryrootpathother") %></h2>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded" width="40%"><%=bundle.getString("label.cardissue.DepartmentName") %></th>
            <th scope="col" class="rounded" width="25%"><%=bundle.getString("label.cardissue.TotalStaffs") %></th>
            <th scope="col" class="rounded-q4" width="25%"><%=bundle.getString("label.cardissue.CardIssued") %></th>
     ...
     <tbody>
			...
			int IssuedCount=0;
	 		...
	 		for(int i=0;i<deptList.size();i++) {
	    		...
	    		String issuedcount=reportquery.cardissuestaffcount(deptid);
		 		...
		 		IssuedCount+=Integer.parseInt(issuedcount);
	 		   	...
	 		   	out.println("<td class='tablelight'><a href='#' depid='"+deptid+"' totalcount='"+totalcount+"' issuedcount='"+issuedcount+"' deptname='"+deptname+"' onclick='viewdepartment(this)' >"+deptname+"</a></td>");
	 		  if(!totalcount.equals("null") && !totalcount.equals("0"))
	 		  {
	 		   	out.println("<td class='tablelight'><font size='4'>"+totalcount+"</font></td>");
	 		  }
	 		  else
	 		  {
		 		   	out.println("<td class='tablelight'>"+totalcount+"</td>");

	 		  }
	 		 if(!issuedcount.equals("null") && !issuedcount.equals("0"))
	 		 {
	 		   	out.println("<td class='tablelight'><font size='4'>"+issuedcount+"</font></td>"); 
	 		 }
	 		 else
	 		 {
		 		   	out.println("<td class='tablelight'>"+issuedcount+"</td>"); 
	 		...
	 		{
				out.println("<tr align='center' class='tablebold'><td colspan='4'><font color='red'>"+bundle.getString("label.cardissue.datanotfound")+"</font></td></tr>");
	 		}
