...
function backfun()
{
document.f1.action= "./UserNotUtilStaff.jsp";
document.f1.submit();
}
...
%>
    <h2><%=bundle.getString("label.usernotutil.entryrootpathother") %> / <%=departmentname %> </h2>
...
<%
	...
	deptList =reportquery.departmentvisestaffNotUtil(departid);
...
<table id="rounded-corner" border=0 
    			...
    			<tr>
    				<td class=tablebold >
    				<%=bundle.getString("label.usernotutil.DepartmentName")%>
    				</td>
    				...
    				<%=bundle.getString("label.usernotutil.TotalStaffs")%>
    				</td>	
    			...
    			<tr>
	    			...
	    			<%=bundle.getString("label.usernotutil.NotUtilizedUsers")%>
	    			</td>
	    			<td class=tablebold>:</td>
	    			<td >
	    			<%=issuedcount%>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded" width="30%"><%=bundle.getString("label.usernotutil.StaffName") %></th>
            <th scope="col" class="rounded" width="20%"><%=bundle.getString("label.usernotutil.Designation") %></th>
            <th scope="col" class="rounded" width="15%"><%=bundle.getString("label.usernotutil.issuedstatus") %></th>
            <th scope="col" class="rounded-q4" width="35%"><%=bundle.getString("label.usernotutil.dateofissued") %></th>
        </tr>
    </thead>
    <tfoot>  
    	<tr>
		...
		try
		{
			if(deptList.size()!=0)
			{
 			...
 			if(agentid!=null &&  !agentid.equals("null") )
 			{
	 		   	...
	 		   	out.println("<td>"+Staffname+"</a>,&nbsp;"+qualification+" </td>");
 			...
 			{
 			   	out.println("<tr>");
	 		   	out.println("<td align=left><font color='red'>"+(i+1)+"</font></td>");
	 		   	out.println("<td><font color='red'>"+Staffname+"</font></a><font color='red'>,</font>&nbsp;<font color='red'>"+qualification+"</font> </td>");
	 		   	out.println("<td><font color='red'>"+rolename+"</font></td>");
	 		   	
	 		   	out.println("<td><font color='red'>Not Issued</font></td>");
	 		   	out.println("<td><font color='red'>"+issueddate+"</font></td>");
 			}
	...
	%>
	 ...
	 <a href="#bbsb" name="bbsb" onclick=backfun() class="bt_blue" align=center><span class="bt_blue_lft"></span><strong><%= bundle.getString("button.usernotutil.back")%></strong><span class="bt_blue_r"></span></a>
