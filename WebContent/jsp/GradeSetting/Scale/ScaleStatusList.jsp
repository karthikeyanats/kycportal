...
<title>Know Your Child - <%=bundle.getString("label.scalecreation.trasklisttitle") %> | Powered by i-Grandee</title>
...
if(status=="A")
{
		statusword="Restore the Scale...?";
}
else if(status=="X")
{
		statusword="Delete the Scale...?";
}

if(confirm("Do You Want to "+statusword))
{
	document.scaleForm.reqscaleid.value = obj.getAttribute("scaleid");
	document.scaleForm.requeststatus.value = obj.getAttribute("reqstatus");
...
function loadStandard()
{
	...
	document.scaleForm.action="ScaleStatusList.jsp";
	document.scaleForm.submit();
}
...
<%@ include file="../../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.scalecreation.trasklistrootpath") %></h2>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded" width="10%"><%=bundle.getString("label.scalecreation.Delete") %></center></th>
            <th scope="col" class="rounded-q4" width="10%"><%=bundle.getString("label.scalecreation.Restore") %></center></th>
    ...
    <tfoot>
     ...
     try
     {
    	...
    	//  instituteid = (String)session.getValue("instituteid");
    	  if(request.getParameter("reqboardid") != null)
    	  {
    	  	...
    	  	scaleList = gradescaleQuery.listScaleList(instituteid,reqboardid,"T");
    	  }
    	...
    	//standardid, standardname, boardid, boardname, mediumname, mediumid
    	 if(scaleList!=null && scaleList.size()>0)
            ...
            <td><%=stdcreateddate %> </td>
               <td width="5%"> <a href="#"  scaleid="<%=scaleid %>" reqstatus="X" onclick="gotoAction(this)"> <img src="<%=request.getContextPath()%>/jsp/kycpanel/images/user_logout.png" alt="delete" title="Delete" border="0" /></a></td>
               <td width="5%"><a href="#" scaleid="<%=scaleid %>" reqstatus="A" onclick="gotoAction(this)"><img src="<%=request.getContextPath()%>/jsp/kycpanel/images/16-undo1.png" alt="restore" title="Restore" border="0" /> </a></td> 
				...
				<%
    	 		...
    	 		{
    	 			out.println("<tr align='center'><td colspan=5>Select Board-Medium</td></tr>");
    	 		}
     ...
     %>
          ...
          <a href="./ScaleList.jsp" class="bt_blue"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.scalecreation.BacktoActiveStandardList") %></strong><span class="bt_blue_r"></span></a>
