...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.schedule.lessontrashpreview")%></h2>
<%@ page language="java"
...
<%
		...
		stndardList					= schedule.viewAssignmentTopics(lessonscheduleid);
		sectionList					= schedule.viewDiaryDetails(lessonscheduleid);
		workList					= schedule.viewWorkDoneDetails(lessonscheduleid);
...
%>
	...
	<form name="Schedule_Form" action="" method="post">
		    ...
		    <tr>
		    	<td colspan='2'>
					 <table width='100%' align='center' border='0' cellpadding='5' cellspacing='0'>
						...
						<tr>
							<td width='20%' class='tablebold'><%=bundle.getString("label.schedule.lessonname")%></td>
							<td width='1%' class='tablebold'>:</td>
							<td class='tablelight'>
								<%=lessonname%>
							</td>
		...
		<table width='90%' align='center' border='0' >
					...
					<table id="rounded-corner" border=0 summary="Department List" width="80%">
					    <thead>
					    	<tr>
					            ...
					            <th scope="col" class="rounded" 		width="89%"><%=bundle.getString("label.schedule.assignmenttopic")%></th>
							...
							try{
								...
								if(stndardList != null && stndardList.size()>0){
									for(int y=0;y<stndardList.size();y++){
										...
										out.print("<tr><td>"+(y+1)+".</td><td colspan='2'>"+hashmap.get("questionname")+"</td></tr>");
									}
								}
								else{
									out.println("<tr align='center' class='tablebold'><td colspan='3'><a class='nodata'>"+bundle.getString("label.schedule.datanotfound")+"</a></td></tr>");
								}
							}
							catch(Exception e){
				...
				<br>
					<table id="rounded-corner" border=0 summary="Department List" width="80%">
					    <thead>
					    	<tr>
					        	<th scope="col" class="rounded-company" width="10%"><%=bundle.getString("label.schedule.slno")%></th>
					            <th scope="col" class="rounded" 		width="50%"><%=bundle.getString("label.schedule.dairyname")%></th>
							...
							try{
								...
								if(sectionList != null && sectionList.size()>0){
									for(int y=0;y<sectionList.size();y++){
										...
										out.print("<tr><td>"+(y+1)+".</td><td>"+hashmap.get("subject")+"</td><td>"+hashmap.get("workdetails")+"</td></tr>");
									}
								}
								else
									out.print("<tr><td colspan='3' align='center'><a class='nodata'>"+bundle.getString("label.schedule.datanotfound")+"</a></td></tr>");
							}
							catch(Exception e){
				...
				<br>
					<table id="rounded-corner" border=0 summary="Department List" width="80%">
					    <thead>
					    	<tr>
					        	<th scope="col" class="rounded-company" width="10%"><%=bundle.getString("label.schedule.slno")%></th>
					            <th scope="col" class="rounded" 		width="89%"><%=bundle.getString("label.schedule.workdonedetails")%></th>
					            <th scope="col" class="rounded-q4" 		width="1%"></th>
					        </tr>
					    </thead>
						<%
							...
							try{
								...
								if(workList != null && workList.size()>0){
									for(int y=0;y<workList.size();y++){
										hashmap			= (HashMap)workList.get(y);
										out.print("<tr><td>"+(y+1)+".</td><td colspan='2'>"+hashmap.get("workdonedetails")+"</td></tr>");
									}
								}
								else
									out.print("<tr><td colspan='3' align='center'><a class='nodata'>"+bundle.getString("label.schedule.datanotfound")+"</a></td></tr>");
			...
			<%
				if((stndardList != null && stndardList.size()>0) || (sectionList != null && sectionList.size()>0) || (workList != null && workList.size()>0)){
					out.print("<tr><td><h3>You Can not Trash this Section if it has active Assignment,Dairy and Work Done Details.</h3></td></tr>");
				}
