...
<script type="text/javascript">
	...
	function publishStandard()
	{
		...
		if(check_ForParticularElements(document.subjectSearch_Form.boardname,"select",document.subjectSearch_Form.examname,"select",document.subjectSearch_Form.course,"select",document.subjectSearch_Form.subject,"select"))
		{
    ...
    <%} %>
		<form name="subjectSearch_Form" action="" method="post">
		    ...
		    <table id="rounded-corner" border=0 width="100%">
		    	<thead>
		    		<tr>
		            	<th scope="col" class="rounded-company" width="100%"><%=bundle.getString("label.topperweaker.rangeheader")%></th>
...
<%
		...
		if(session.getValue("sessionid") != null && request.getParameter("boardname") != null  && request.getParameter("examname") != null && request.getParameter("course") != null)
...
%>
	...
	<input type="hidden" name="maxmarkvar" value="">  
		...
		<table width='100%' align='center' id="rounded-corner1" border='0' cellpadding='5' cellspacing='0' >
			...
			<tr >
				<td class=tablebold><%=bundle.getString("label.topperweaker.subject")%> <font color="red">*</font></td>
				<td width=1px class=tablebold>:</td><td colspan=3><select name=subject validate="Subject " class=tablelight onchange="maxmark()" >
		...
		<h2><%=bundle.getString("label.topperweaker.range")%></h2>
