...
<%@ include file="../include/header.jsp" %>
	...
	<%
		String existing		= request.getParameter("status");
		String displaystr	= "";
		if(existing != null && existing.equals("existing"))
			displaystr		= "Existing Diary Details";
		else
			displaystr		= "Current Diary Details";
	
	%>
    <div class="right_content">

    <h2><%=bundle.getString("label.diary.parentroot4")%></h2>
	...
	<form name='workallotment' method='post'>
		    ...
		    <tr>		
		    	<td colspan='2'>					
					...
					<%
						...
						out.print("<tr><td width='17%' class=tablebold >"+bundle.getString("label.diary.StudentName")+"</td><td width='1%'>:</td><td>"+session.getValue("studentname")+"</td></tr>");
						out.print("<tr><td class=tablebold >"+bundle.getString("label.diary.AcademicYear")+"</td><td>:</td><td>"+session.getValue("sessionname")+"</td></tr>");
						if(session.getValue("groupname") == null)
							out.print("<tr><td class=tablebold >"+bundle.getString("label.diary.StandardName")+"</td><td>:</td><td>"+session.getValue("standardname")+"</td></tr>");
						else
							out.print("<tr><td class=tablebold >"+bundle.getString("label.diary.StandardName")+"</td><td>:</td><td>"+session.getValue("standardname")+"&nbsp;&nbsp;[&nbsp;"+session.getValue("groupname")+"&nbsp;]</td></tr>");
						out.print("<tr><td class=tablebold >"+bundle.getString("label.diary.SectionName")+"</td><td>:</td><td>"+session.getValue("sectionname")+"</td></tr>");
						out.print("<tr><td class=tablebold >"+bundle.getString("label.diary.BoardName")+"</td><td>:</td><td>"+session.getValue("boardname")+"</td></tr>");
						out.print("<tr><td class=tablebold >"+bundle.getString("label.diary.MediumName")+"</td><td>:</td><td>"+session.getValue("mediumname")+"</td></tr>");
						out.print("</table>");
					%>	
				</td>
			</tr>
			...
			<table id="rounded-corner" border=0 summary="Department List" width="100%">
			    <thead>
			    	<tr>
			            ...
			            <th scope="col" class="rounded-company" width="98%"><%=bundle.getString("label.diary.ParentRemarks")%><font color='red'>*</font></th>
			            <th scope="col" class="rounded-q4" width="2%"></th>
			        </tr>
			    </thead>
				<tr>
					<td colspan='2'>							
						<input type=hidden name='tempann' value='500'><textarea rows="5" cols="75" class="tablelight" name="studremarks" validate="Parent Remarks" onkeydown="calculateRemaining(this,500,'remaining')" onkeyup="calculateRemaining(this,500,'remaining')" ></textarea><br><span id='remaining'><font color='red'>500 characters remaining</font></span>
					</td>
				</tr>
			</table>
			...
			<table id="rounded-corner" border=0 summary="Department List" width="100%">
			    <thead>
			    	<tr>
			            ...
			            <th scope="col" class="rounded-company" width="98%"><%=bundle.getString("label.diary.ParentRemarks")%> on <%=parentdate%></th>
			            <th scope="col" class="rounded-q4" width="2%"></th>
			        </tr>
			    </thead>
				<tr>
		...
		%>
			...
			<table id="rounded-corner" border=0 summary="Department List" width="100%">
			    <thead>
			    	<tr>
			            <th scope="col" class="rounded-company" width="98%"><%=bundle.getString("label.diary.FacultyRemarks")%> on <%=staffdiaryreplydate%></th>
			            <th scope="col" class="rounded-q4" width="2%"></th>
			        </tr>
			    </thead>
				<tr>
