...
function loadResult()
{
	document.submitlessonform.action="<%=request.getContextPath()%>/jsp/Parent/Opreation_result_withrequest.jsp";
...
try
{
	...
	passflag =  loginobj.checkPasswordParent(username,oldpassword);
	if(passflag>1)
	{
		cnt = passwdquery.updateChangePasswordforparent(username,confirmpassword);
	}
}catch(Exception e){//System.out.println("Exception in Submit Password"+e.getMessage());
}
		...
		if(passflag >1)
		{
			...
			resultword="Operation Failed";
			 ...
			 return_file = "./changepasswd.jsp";

		}
		else
		{
			resultword="Entered Wrong Old Password";
			 return_file = "./oldpassword.jsp";
