...
var url = "";
	   if(flag=="standard")
	   {
		   var val22=val.split("@");
		   val=val22[0];
	  	  url = "./attendancestaffinfo.jsp?action=standard&sessionid="+val+"&boardid="+val1+"&instituteid="+val2+"&dat="+dat;
	 ...
	 else if(flag=="section")
	 {
	 		...
	 		document.Attendance_Form.todaydate.value	= document.Attendance_Form.standard.options[document.Attendance_Form.standard.options.selectedIndex].getAttribute("startdate")+","+document.Attendance_Form.standard.options[document.Attendance_Form.standard.options.selectedIndex].getAttribute("enddate")+","+document.Attendance_Form.todaydate.value;
		   	...
		   	url = "./attendancestaffinfo.jsp?action=section&standardscheduleid="+val1+"&instituteid="+val2+"&dat="+dat;
	   }
	     else
	   {
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.staffschedule.attendanceentry")%></h2>
<%@ page language="java"
...
<%
		...
		ArrayList BoardList = registrationQuery.loadBoardForStaff(userid,instituteid,sessionid);
					...
					<select name='session'  id='session'   validate='Academic Year' >
						...
						if(sessionList != null && sessionList.size()>0)
							for(int y=0;y<sessionList.size();y++){
								...
								if(hashmap.get("sessionstatus").equals("A") && hashmap.get("sessionname").equals(session.getValue("sessionname")))
									out.print("<option value='"+hashmap.get("sessionid")+"@"+hashmap.get("sessionstatus")+"' selected>"+hashmap.get("sessionname")+"</option>");
							}
						%>
					</select>
