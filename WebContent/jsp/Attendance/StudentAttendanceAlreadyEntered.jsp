...
<script type="text/javascript">
	...
	if(document.Attendance_Form.sessionusercat.value == "SA" || document.Attendance_Form.sessionusercat.value == "MA")
		document.Attendance_Form.action="./AttendanceEntry.jsp";
	else
		document.Attendance_Form.action="./AttendanceEntryForStaff.jsp";
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.staffschedule.studentattendancelist")%></h2>
<%@ page language="java"
	import="java.util.ResourceBundle,java.util.ArrayList,java.util.HashMap"%>
	<%@ page import="com.iGrandee.Common.DateTime" %>
	
<%@ page language="java" import="com.iGrandee.Attendance.AttendanceQuery,com.iGrandee.Attendance.AttendanceBean,com.iGrandee.Common.DateTime"%>
<%
		...
		String sessionusercat			= (String)session.getValue("usercategory");
...
<form name="Attendance_Form" action="" method="post">
		...
		<input type="hidden" name="sessionusercat" value="<%=sessionusercat%>">
		...
		<br>
			...
			<tr>
				<td width='20%' class='tablebold'>
					...
					<table id="rounded-corner" border=0  width="100%">
					    <thead>
					    	<tr>
					            ...
					            <th scope="col" class="rounded" 		width="10%"><%=bundle.getString("label.schedule.rollno")%></th>
					            <th scope="col" class="rounded"   	 width="10%"><%=bundle.getString("label.schedule.attendance")%></th>
					            <th scope="col" class="rounded"   	 width="15%"><%=bundle.getString("label.common.createdby")%></th>
					        	<th scope="col" class="rounded-q4"   align="center" width="35%"><%=bundle.getString("label.schedule.dateofcreation")%></th>
						...
						if(studentList != null && studentList.size()>0){
							String allocationids  = request.getParameter("resultids");
							String[] allocationidArr = null;
							//out.print(request.getParameter("resultids"));
							allocationidArr = allocationids.split(",");
							for(int y=0,j=1;y<studentList.size();y++,j++){
								...
								if(allocationidArr!=null && allocationidArr.length>0){
									for(int z=0;z<allocationidArr.length;z++){
										if(allocationidArr[z].equals(studallocationid)){
								out.print("<tr>");
								...
								ArrayList Studstate=attencequery.getattendanceforstudentstaff(studallocationid,searchdate);
								String status="";
								String attendanceid="";
								String staffname = "";
								...
								HashMap studmap=(HashMap)Studstate.get(0);
								 ...
								 staffname = studmap.get("staffname")+"";
								 dateofcreation = studmap.get("dateofcreation")+"";
								 attendanceby = studmap.get("createdby")+"";
								...
								else
								{
									...
									if(status.equals("P"))
										...
										out.print(status);
									out.print("</td>");
									out.print("<td>"+staffname+"</td>");
									out.print("<td>"+dateofcreation+"</td>");
								}
					...
					%>
					<td valign=top>
						Note:</td><td>&nbsp;</td><td> P-Present<br>A-Absent<br>OD-OnDuty<br>Attendance already entered for the above Students</br></td>
						</tr>
					</table>
