...
{
 	//window.open("./printStdwise.jsp?startdate="+document.MainForm.startdate.value+"&enddate="+document.MainForm.enddate.value+"&flage_temp="+document.MainForm.flage_temp.value,"Msg","top=250,left=350,help:no,center:no,status:no,width=650,height=800,scrollbars=1;")
 	window.open("./printStdwise.jsp?req_sessionname="+document.MainForm.req_sessionname.value+"&req_boardname="+document.MainForm.req_boardname.value+"&req_standardnamegp="+document.MainForm.req_standardnamegp.value+"&req_sectionname="+document.MainForm.req_sectionname.value+"&req_sectionschedid="+document.MainForm.req_sectionschedid.value+"&flage_temp="+document.MainForm.flage_temp.value,"Msg","top=250,left=350,help:no,center:no,status:no,width=650,height=800,scrollbars=1;")
}
...
String sectionscheduleid=request.getParameter("req_sectionpubid");
...
ArrayList JournalList=new ArrayList();
	...
	{ 
		BookList=query_object.get_All_BookExceedDuedateStdwise(startdate,enddate,sectionscheduleid,"bookMaster",instituteid); 
	}
	else if(flage_temp.equals("Journal"))
	{ 
		JournalList=query_object.get_All_BookExceedDuedateStdwise(startdate,enddate,sectionscheduleid,"Journal",instituteid);  
...
<%@ include file="../../include/header.jsp" %>
	...
	if(flage_temp.equals("Book"))
	{
    out.print("<h2>"+bundle.getString("label.bookexceedduedatestd.entryrootpath1")+"</h2>");
    
	}
	else if(flage_temp.equals("Journal"))
	{
	    out.print("<h2>"+bundle.getString("label.bookexceedduedatestd.entryrootpath2")+"</h2>");
...
<table id="rounded-corner" border=0 width="100%">
    ...
    <tbody>
			...
			<tr>
				...
				<td class=tablebold><%=bundle.getString("label.exam.Board")%></td>
				<td class=tablebold width="2%">:</td>
				<td><%=request.getParameter("req_boardname")%>					
				</td>
			</tr>
			<tr>
				<td class=tablebold><%=bundle.getString("label.exam.Standard")%> </td>
				<td class=tablebold width="2%">:</td>
				<td><%=request.getParameter("req_standardnamegp")%>
				</td>

				<td class=tablebold><%=bundle.getString("label.exam.Section")%> </td>
				<td class=tablebold width="2%">:</td>
				<td><%=request.getParameter("req_sectionname")%>
				</td>
			</tr>    
...
%>
		...
		<a href="StdwiseBookExceedDue.jsp" class="bt_blue" ><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.datewisebooksearch.Back") %> </strong><span class="bt_blue_r"></span></a>
		...
		<input type="hidden" name="req_sectionschedid" value="<%=request.getParameter("req_sectionpubid")%>">
