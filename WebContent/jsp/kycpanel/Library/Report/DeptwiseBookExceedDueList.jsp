...
function printReport()
{
 	...
 	window.open("./printDeptwise.jsp?req_deptname="+document.MainForm.req_deptname.value+"&req_deptid="+document.MainForm.req_deptid.value+"&flage_temp="+document.MainForm.flage_temp.value,"Msg","top=250,left=350,help:no,center:no,status:no,width=650,height=800,scrollbars=1;")
}
...
String departmentid=request.getParameter("req_deptid");
...
ArrayList JournalList=new ArrayList();
	...
	{  
		BookList=query_object.get_All_BookExceedDuedateDeptwise(startdate,enddate,departmentid,"bookMaster",instituteid); 
	}
	else if(flage_temp.equals("Journal"))
	{
		JournalList=query_object.get_All_BookExceedDuedateDeptwise(startdate,enddate,departmentid,"Journal",instituteid);  
...
<%@ include file="../../include/header.jsp" %>
	...
	if(flage_temp.equals("Book"))
	{
    out.print("<h2>"+bundle.getString("label.bookexceedduedatedeptwise.entryrootpath1")+"</h2>");
    
	}
	else if(flage_temp.equals("Journal"))
	{
	    out.print("<h2>"+bundle.getString("label.bookexceedduedatedeptwise.entryrootpath2")+"</h2>");
...
<table id="rounded-corner" border=0 width="100%">
    ...
    <tbody>
			...
			<tr>
				<td class=tablebold align=right>Department</td>
				<td align=left>&nbsp;<b>:</b>&nbsp;<%=request.getParameter("req_deptname")%></td>
			</tr>
	</tbody>			
   ...
   <table id="rounded-corner1" border=0 summary="Department List" width="100%">
    <thead>
    	<tr class=tableheading  >
            ...
            <th scope="col" class="rounded" width="10%" colspan=<%=3 %>><%=bundle.getString("label.bookreturn.ReturnBy")%></th>
...
%>
		...
		<a href="DeptwiseBookExceedDue.jsp" class="bt_blue" ><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.datewisebooksearch.Back") %> </strong><span class="bt_blue_r"></span></a>
		...
		<input type="hidden" name="req_deptname" value="<%=request.getParameter("req_deptname")%>">
		<input type="hidden" name="req_deptid" value="<%=request.getParameter("req_deptid")%>">
