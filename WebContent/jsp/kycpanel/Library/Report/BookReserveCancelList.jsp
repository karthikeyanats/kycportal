...
{
 	window.open("./printReserve.jsp?startdate="+document.MainForm.startdate.value+"&enddate="+document.MainForm.enddate.value+"&flage_temp="+document.MainForm.flage_temp.value,"Msg","top=250,left=350,help:no,center:no,status:no,width=650,height=800,scrollbars=1;")
...
ArrayList JournalList=new ArrayList();
	...
	{
		BookList=query_object.get_All_BookReserveCancel(startdate,enddate,rackwise,"bookMaster",instituteid);
	}
	else if(flage_temp.equals("Journal"))
	{
		JournalList=query_object.get_All_BookReserveCancel(startdate,enddate,rackwise,"Journal",instituteid);  
...
<%@ include file="../../include/header.jsp" %>
	...
	if(flage_temp.equals("Book"))
	{
    out.print("<h2>"+bundle.getString("label.bookreservecancel.entryrootpath1")+"</h2>");
    
	}
	else if(flage_temp.equals("Journal"))
	{
	    out.print("<h2>"+bundle.getString("label.bookreservecancel.entryrootpath2")+"</h2>");
    ...
    <thead>
    	<tr class=tableheading  >
            ...
            <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookreturn.AccessNo") %></th>
             ...
             <th scope="col" class="rounded" width="10%"  rowspan=2><%=bundle.getString("label.bookreservecancel.ReserveDate") %></th>
            <th scope="col" class="rounded" width="10%" colspan=<%=3 %>><%=bundle.getString("label.bookreservecancel.ReservedBy")%></th>
				...
				try
				{
			     if(BookList!=null && BookList.size()>0)
			     {
			    	 ...
			    	 for(int i=0,j=1;i<BookList.size();i++,j++)
			    	 {
			    	     ...
			    	     String returndate=BookListMap.get("reservedate")+"";
			    		...
			    		{   
			    		 out.print("<td class='tablelight' >"+BookListMap.get("reservedate")+"</td>"); 
			    		}
...
%>
		...
		<a href="BookReserveCancel.jsp" class="bt_blue" ><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.datewisebooksearch.Back") %> </strong><span class="bt_blue_r"></span></a>
