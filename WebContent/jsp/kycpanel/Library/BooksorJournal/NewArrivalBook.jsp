...
<html>
<head>
<title><%=bundle.getString("label.newarrivalbookmasterlist.entrytitle") %></title>
...
String today				= datebeans.showTime("calendar");
String flage=request.getParameter("flage")+"";
String prevdate="";
String prevdate_temp_final="";
BookList_book=query_object.get_NewArrival_Books(sessionid,"bookMaster",instituteid);     
prevdate=query_object.get_StartDate(today);
String[] prevdate_temp=prevdate.split("-");
 String as[] = {
            "0","Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "sep", "Oct",
            "Nov", "Dec"
        };

 		prevdate_temp_final=prevdate_temp[2]+"-"+as[Integer.parseInt (prevdate_temp[1])]+"-"+prevdate_temp[0];
...
if(flage!=null)
{
	...
	{
		BookList_journal=query_object.get_NewArrival_Journals(sessionid,"Journal",instituteid);  
...
<%@ include file="../../include/header.jsp" %>
    ...
    out.print("<h2>"+bundle.getString("label.newarrivalbookmasterlist.entryrootpath")+" ( between "+prevdate_temp_final+" and "+today+" )</h2>");
...
<tr><td colspan=2 class="tablebold">New Arrival Book List</td></tr>
</table>
 	<table id="rounded-corner" border=0  width="100%">
    <thead>
    	<tr> 
        	<th scope="col" class="rounded-company" width="5%"><%=bundle.getString("label.newarrivalbookmasterlist.SL.No") %></th>
        	<th scope="col" class="rounded" width="20%"><%=bundle.getString("label.newarrivalbookmasterlist.Title") %> </th>
            <th scope="col" class="rounded" width="20%"><%=bundle.getString("label.newarrivalbookmasterlist.Author") %> </th>
            <th scope="col" class="rounded" width="20%"><%=bundle.getString("label.newarrivalbookmasterlist.Edition") %> </th>
            <th scope="col" class="rounded" width="15%"><%=bundle.getString("label.newarrivalbookmasterlist.Price") %> </th>
      	    <th scope="col" class="rounded-q4" width="25%"><%=bundle.getString("label.newarrivalbookmasterlist.DateofCreation") %> </th>
...
else if(flage.equals("Journal"))
{
		%>
<table width="100%" align="center">
<tr><td colspan=2 class="tablebold">New Arrival Journal List</td></tr>
</table>
		<table id="rounded-corner" border=0  width="100%">
    <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="5%"><%=bundle.getString("label.newarrivaljournallist.SL.No") %></th>
        	<th scope="col" class="rounded" width="20%"><%=bundle.getString("label.newarrivaljournallist.BookName") %> </th>
            <th scope="col" class="rounded" width="20%"><%=bundle.getString("label.newarrivaljournallist.issueno") %> </th>
            <th scope="col" class="rounded" width="20%"><%=bundle.getString("label.newarrivaljournallist.volumeno") %> </th>
            <th scope="col" class="rounded" width="15%"><%=bundle.getString("label.newarrivaljournallist.Price") %> </th>
      	    <th scope="col" class="rounded-q4" width="25%"><%=bundle.getString("label.newarrivaljournallist.DateofCreation") %> </th>
