...
<title>Know Your Child - <%=bundle1.getString("label.feespaiddetails.title")%> | Powered by i-Grandee</title>
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle1.getString("label.feespaiddetails.rootpathforreport")%></h2>


<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            <th scope="col" class="rounded-company" width="90%"  ><%=bundle1.getString("label.feespaiddetails.SelectedInformation")%> </th>
...
<tr>
	<td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.AcademicYear")%></td>
	<td class=tablebold>:</td>
	<td class=tablelight ><%=yearofsession %></td>
	<td class=tablebold><%=bundle1.getString("label.feespaiddetails.Board")%> </td>
	<td class=tablebold>:</td>
	<td  class=tablelight><%=boardName %></td>
</tr>
<tr><td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.Standard")%></td>
<td class=tablebold>:</td>
<td class=tablelight><%=request.getParameter("standardnamegp") %></td>
<td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.Term")%></td>
<td class=tablebold>:</td>
<td class=tablelight><%=request.getParameter("req_termfeesname")%></td>
</tr>
<tr><td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.DueDate")%></td>
<td class=tablebold>:</td>
<td class=tablelight><%=request.getParameter("req_duedate")%></td>
<td class=tablebold width="25%"><%=bundle1.getString("label.feespaiddetails.TermFeesAmount")%></td>
<td class=tablebold>:</td>
<td class=tablelight><%=formattedTotTermFeeCurrency %></td>
</tr>
<%
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="7%"><%=bundle1.getString("label.feespaiddetails.SL.No")%></th>
            <th scope="col" class="rounded" width="35%"><%=bundle1.getString("label.feespaiddetails.StudentName")%></th>
            <th scope="col" class="rounded" width="15%"><%=bundle1.getString("label.feespaiddetails.Section")%></th>
            <!--  <th scope="col" class="rounded" width="15%">Application No.</th>-->
            <th scope="col" class="rounded" width="10%"><%=bundle1.getString("label.feespaiddetails.RollNo")%>.</th>
            <th scope="col" class="rounded" width="12%">Scholoarship</th>
            <th scope="col" class="rounded-q4" width="12%"><%=bundle1.getString("label.feespaiddetails.PaidAmount")%></th>
            <!--  <th scope="col" class="rounded-q4" width="15%">Pending Amount</th>-->                       
        </tr>
    </thead>
    <tfoot>
    	<tr>
	...
	if(pendingFeesList.size()!=0)
	{
		...
		//	String RemAmount				=	(String) map.get("RemAmount");
			//studentappamount(studentapprovalId);
		    ...
		    //Long feesPaidObj = Long.valueOf(feesPaid);
		    ...
		    //Long remAmountObj = Long.valueOf(RemAmount);
			...
			if(feeStatus.equals("Paid") || feeStatus.equals("Excess")){
				count=count+1;	
				...
				out.print("<tr><td>"+(count)+"</td><td>"+applicantName+"</td><td>"+sectionName+"</td><td>"+rollNo+"</td><td>"+formattedscholarshipamountCurrency+"</td>");
						/* if(feeStatus.equals("Excess")){
				out.print("<td>"+formattedFeesPaidCurrency+"("+remAmountObj+")</td>");
						}else
						{ */
							out.print("<td title='"+remAmountObj+"'>"+formattedFeesPaidCurrency+"</td>");
						//}
...
%>
		...
		<a href="#" class="bt_blue" onclick="goBack()"><span class="bt_blue_lft"></span><strong><%=bundle1.getString("button.feespaiddetails.Back")%></strong><span class="bt_blue_r"></span></a>
		<a href="#" class="bt_green" onclick='printfun()'><span class="bt_green_lft"></span><strong><%=bundle1.getString("button.feespaiddetails.Print")%></strong><span class="bt_green_r"></span></a>
		<%
