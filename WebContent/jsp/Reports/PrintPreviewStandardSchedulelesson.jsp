...
<title>Know Your Child - Standard Schedule lesson List | Powered by i-Grandee</title>
...
InstitutionQurey insquery=new InstitutionQurey();
		...
		String subjectscheduleid	= request.getParameter("req_subjectscheduleid");
		...
		if(standardscheduleid != null && boardname != null) {
			 subjectList		= schedule.viewLessons(subjectscheduleid);
			}
%>
...
<form name="Schedule_Form" action="" method="post">
		...
		<tr><td width="35%" valign=top align=center><%=bundle.getString("label.schedule.scheduledlesson")%>
		</td></tr>
		<tr><td valign=top><br>
		    	...
		    	<td  align= center>
					<table width='90%' align='center' border='0' cellpadding='5' cellspacing='0'>
						...
						<td width='20%' class='tablebold' align='left'><%=bundle.getString("label.schedule.standardname")%></td>
							...
							<td width='20%' class='tablebold' align='left'><%=bundle.getString("label.schedule.subjectname")%></td>
							<td width='1%' class='tablebold' >:</td>
							<td class='tablelight'   align=left>
								<%=request.getParameter("req_subjectname") %>
							</td>
						</tr>
						<%
					...
					<table id="rounded-corner" border=1 summary="Department List" width="100%" cellspacing=0 cellpadding=5 bordercolor=grey>   
					    <thead>
					    	<tr  class='tableheading'>
					        	<th width="10%"  align=left><%=bundle.getString("label.schedule.slno")%></th>
					            <th	width="40%"  align=left><%=bundle.getString("label.schedule.lessonname")%></th>
					            <th	width="40%"  align=left><%=bundle.getString("label.schedule.desc")%></th>
					        </tr>
					    </thead>
						<%
					    try{
					    	if(subjectList != null && subjectList.size()>0){
					    		...
					    		for(int y=0;y<subjectList.size();y++){
					    			...
					    			out.print("<tr class='tablelight' ><td>"+(count1++)+".</td><td>"+hashmap.get("lessonname")+"</td><td>"+hashmap.get("lessondescription")+"</td>");
					    			datacount++;
					    		}
					    	}
					    	else{
				    			...
				    			//out.print("<tr><td colspan='3' class='tablelight' align='center'>"+bundle.getString("label.schedule.datanotfound")+"</td></tr>");
					    	}
					    }catch(Exception e){
