...
function goBack()
{
	...
	function printfun()
	{
	window.open("PrintPreviewFeesStructureView.jsp?&req_sessionname="+document.applicationForm.req_sessionname.value+"&boardname="+document.applicationForm.req_boardname.value+"&standardpublishid="+document.applicationForm.standardpublishid.value+"&req_standardnamegp="+document.applicationForm.standardnamegp.value+"&req_sessionstatus="+document.applicationForm.req_sessionstatus.value,"Msg","top=250,left=350,help:no,center:no,status:no,width=650,height=650,scrollbars=1;")
	}	
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle1.getString("label.feesstructureview.rothpathinreport")%></h2>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
	...
	<tr>
	<td class=tablebold width="25%"><%=bundle1.getString("label.feesstructureview.Standard")%></td>
	<td class=tablebold>:</td>
	...
	<tr class='tablebold' width="25%" height='23'>
			<td  class='tablebold' width='20%' valign=top ><%=bundle1.getString("label.feesstructureview.ArchivalDetails")%></td>
			
			<td colspan=3 valign=top class='tablelight' align=left><h2><%=bundle1.getString("label.feesstructureview.ThisisanArchivedYear")%></h2></td>
	</tr>
	<%
	} 
	%>					
...
%>  
		    ...
		    <tr class=tableheading><td> Term Name</td><td align=right><%=bundle1.getString("label.feesstructureview.Amount")%></td></tr>
		   	    	<%
				...
				headtotalamount +=Integer.parseInt(termfeesamount);
					...
					//out.println("<tr><td colspan=2 align=center>Data not found</td></tr>");
				}%>
				</table>
		</td>
		<td width="50%">
   ...
   <tr><td colspan=2 class=tablebold align=center> <%=bundle1.getString("label.feesstructureview.FeesHeadwiseAmountDetails")%></td></tr>
	...
	<%if(termfeesscheduleList.size()>0 || feesheadscheduleList.size()>0){%> 
			<a href="#" class="bt_green" onclick='printfun()'><span class="bt_green_lft"></span><strong><%=bundle1.getString("button.feesstructureview.Print")%></strong><span class="bt_green_r"></span></a>
	<%} %>
