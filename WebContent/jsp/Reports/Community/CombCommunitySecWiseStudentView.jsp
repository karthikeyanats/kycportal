...
function backfun()
{
document.Student_Form.action="./CombCommunitySecView.jsp";
document.Student_Form.submit();
}
...
<%
		...
		operationlabel="/ "+request.getParameter("communityname")+" List";
		
		if(operation.equalsIgnoreCase("both")){
			operationquery = "(cm.exserviceman='Yes' and pm.community="+request.getParameter("communityid")+" and cm.physicallyfit='Yes') and ";
			//operationlabel="/ "+request.getParameter("communityname")+" List";
			operationlabel=operationlabel+" / Both";
		}
		else if (operation.equalsIgnoreCase("exserviceman")){
			operationquery = "(cm.exserviceman='Yes' and pm.community="+request.getParameter("communityid")+") and ";
			operationlabel=operationlabel+" / Son or Daughter of Ex-Service Man";
		}
		else if (operation.equalsIgnoreCase("phychal")){
			operationquery = "(cm.physicallyfit='Yes'  and pm.community="+request.getParameter("communityid")+") and ";
			operationlabel=operationlabel+" / Physically Challenged List";
		}		
		if(!sectionscheduleid.equals("null") && !sectionscheduleid.equals("null") && !operation.equals("null"))
		{
			StudentList=reportQuery.loadstudentforcommunitysectionwithstatus(sectionscheduleid,operationquery,status); 
		}
...
<form name="Student_Form" action="" method="post">
						...
						<input type="hidden" name="condition" value="combcommunitysecview">
		...
		int Totalstudent=0;
							if(StudentList != null && StudentList.size()>0)
								...
								for(int y=0,i=1;y<StudentList.size();y++,i++)
								{
									...
									out.print("<tr align=left valign=top ><td width='5%'>"+i+"</td>");
									out.println("<td align=left valign=top width='25%'><a href='#studenthref' name='studenthref'  studentallocationid='"+studentallocationid+"' personid='"+personid+"' onclick='loadStudentProfile(this)'>"+uname+"</a></td>");
									out.println("<td align=left valign=top width='10%'>"+rollno+"</td>");
