...
function deallocatefun()
{
	...
	if(status)
		{
			if(confirm("Do you Want to Deallocate Online Exam...?"))
			{
				...
				document.question_form.action="./OnlineExamTemplateDeallocationConfirmation.jsp";
	...
	function callBack(){
	  	...
	  	document.question_form.action="OnlineExamTemplateDeallocationSearch.jsp";
	  	document.question_form.submit();
...
<%@ include file="../include/examheader.jsp" %>
    ...
    <h2><%=bundle.getString("label.exam.Deallocationroot_1")%></h2>
    <%
		...
		<table id="rounded-corner" border=0 summary="Department List" width="80%">
		    <thead>
		    	<tr>
		            ...
		            <th scope="col" class="rounded" 		>Assigned Date</th>
		    ...
		    <%
		    	...
		    	if(TemplateList != null && TemplateList.size()>0){
		    		for(int y=0;y<TemplateList.size();y++){
		    			...
		    			out.println(innerexamname+"</td><td>"+assigneddate+"</td><td>");
						if(count>0)		    			
		    			out.println("Subscribed by Student");
						else
