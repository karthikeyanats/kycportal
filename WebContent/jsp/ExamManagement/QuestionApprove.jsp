...
<script type="text/javascript">
	...
	function submitQuestion(){
		...
		if(check_ForParticularElements(document.question_form.academicyear,"select",document.question_form.standard,"select",document.question_form.subject,"select",document.question_form.template,"select",document.question_form.templatetype,"select")){
			...
			document.question_form.action				= "QuestionApprovalView.jsp";
	...
	function startTransaction(str)
	{
		...
		if(str == "board")
		{
			...
			clearObjects("board","standard","subject","template","templatetype");
		...
		else if(str == "standard")
		{
			...
			clearObjects("standard","subject","template","templatetype");
		...
		else if(str == "subject")
		{
			...
			clearObjects("subject","template","templatetype");
		...
		else if(str == "template")
		{
			...
			clearObjects("template","templatetype");

			if(check_ForParticularElements(document.question_form.academicyear,"select",document.question_form.board,"select",document.question_form.standard,"select"))
				url="QuestionPrepareAjax.jsp?action=templateapprove&sessionid="+sessionid+"&subjectid="+subjectid;
			else
				return false;
		}
		else if(str == "templatetype")
		{
			var sessionid 	= document.getElementById("academicyear").value;
			var standardid 	= document.getElementById("standard").value;
			var templateid 	= document.getElementById("template").value;

			clearObjects("templatetype");

			if(check_ForParticularElements(document.question_form.academicyear,"select",document.question_form.board,"select",document.question_form.standard,"select",document.question_form.template,"select"))
				url="QuestionPrepareAjax.jsp?action=templatetypeapprove&sessionid="+sessionid+"&standardid="+standardid+"&templateid="+templateid;
			else
				return false;
		}
	...
	function viewTemplateStatusList()
	{
		document.question_form.action	= "TemplateStatusList.jsp";
		document.question_form.submit();
	}
...
<%@ include file="../include/examheader.jsp" %>
    ...
    <form name="question_form" action="" method="post">
		    ...
		    <tr>
		    	<td colspan='2'>
					<table width='100%' align='center' border='0' cellpadding='5'>
						...
						<tr height=20>
							<td class=tablebold align="left"><%=bundle.getString("label.exam.subjectname")%>-<%=bundle.getString("label.common.subjectcode")%></td>
							<td class="tablebold" valign='top'>:</td>
							<td align="left">
						...
						<tr height=20>
							<td class="tablebold" align="left"><%=bundle.getString("label.exam.templatename")%></td>
							<td class="tablebold" valign='top'>:</td>
							<td align="left"><select id=template name=template validate="Template Name" class=tablelight onChange='startTransaction("templatetype")'>
						...
						<tr height=20>
							...
							<td class="tablebold" valign='top'>:</td>
							<td align="left"><select id=templatetype name=templatetype validate="Template Type" class=tablelight>
		...
		<br>
		 ...
		 <a href="#" class="bt_blue" onclick="viewTemplateStatusList()"><span class="bt_blue_lft"></span><strong><%=bundle.getString("label.exam.templatestatuslist")%></strong><span class="bt_blue_r"></span></a>
		  <a href="#" id="savebut"  class="bt_green" onclick='submitQuestion()'><span class="bt_green_lft"></span><strong><%=bundle.getString("label.exam.display")%></strong><span class="bt_green_r"></span></a>
