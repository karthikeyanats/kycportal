...
<script type="text/javascript">
	function backfun()
	{
	document.StudentReport_Form.action="./sectionpasslist.jsp";
...
<%@ include file="../../include/header.jsp" %>
    ...
    <h2> <%=bundle.getString("label.exampass.secrootpathview")%></h2>
<%@ page language="java"
...
<%
		...
		if(sessionid != null && boardid!= null && standardscheduleid!= null && sectionscheduleid!= null) {
			studentList			= prinicipalquery.loadSectionwisePassListView(standardscheduleid,sectionscheduleid); 
...
%>
		...
		<table width='80%' id=rounded-corner align='center' border='0' >
		 <thead>
		    	<tr>
		            <th scope="col" class="rounded-company" colspan=5 width="100%"><%=bundle.getString("label.exampass.selectinfo")%></th>
					...
					<th scope="col" class="rounded-company"  width="15%"><%=bundle.getString("label.exampass.examname")%></th>
			       	...
			       	int total=0;
						...
						{  
			        		...
			        		//studentListMark			= prinicipalquery.loadSectionwisePassListMarkView(exam_name_id,subjectscheduleid_temp,standardscheduleid,sectionscheduleid);
							if(!subjectList_temp.contains(subjectscheduleid_temp))	
							{		 
							 out.print("<tr>");
							 //out.print("<td><tr><th scope='col' class='rounded'>"+subjectnamenamehash.get("subjectname")+"</th>");
							 subjectscheduleid_temp=(String)subjectnamenamehash.get("subjectscheduleid");
							 //out.print("<td>"+exam_name_id+"--"+subjectscheduleid_temp+"</td>");
							 subjectList_temp.add(subjectscheduleid_temp);
							 subjectnameList_temp.add(subjectnamenamehash.get("subjectname"));
							}
							else
							{								
								//out.print("<td >"+exam_name_id+"--"+subjectscheduleid_temp+"</td></tr>");
							}
							//out.print("<td>"+studentListMark+"</td>");				
							out.print("</td>");
						}
			        	...
			        	for(int z=0;z<subjectList_temp.size();z++){
			        		...
			        		for(int z1=0;z1<examnameList.size();z1++){
			        			studentListMark			= prinicipalquery.loadSectionwisePassListMarkView(examnameList.get(z1)+"",subjectList_temp.get(z)+"",standardscheduleid,sectionscheduleid);
			        			//out.println("<td>"+subjectList_temp.get(z)+"--"+examnameList.get(z1)+"</td>");
