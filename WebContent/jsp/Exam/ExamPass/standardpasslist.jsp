...
<title><%=bundle.getString("label.product.name")%> - <%=bundle.getString("label.exampass.stdwisepass")%>  | Powered by i-Grandee</title>
...
<script type="text/javascript">
	...
	{ 
	if(check_ForParticularElements(document.StudentReport_Form.board,"select",document.StudentReport_Form.standardscheduleid,"select"))
	{
	...
	document.StudentReport_Form.action="./standardpasslistview.jsp"
...
<%@ include file="../../include/header.jsp" %>
    ...
    <h2> <%=bundle.getString("label.exampass.stdrootpath")%></h2>
<%@ page language="java"
...
%>
		...
		<table width='80%' id=rounded-corner align='center' border='0' >
			<thead>
			<tr>
				<th scope="col" class="rounded-company" width="100%" colspan=5><%=bundle.getString("label.exampass.stdwisepass")%></th>
		...
		<tbody ><tr>
				...
				<td class='tablelight' width='40%'>
					...
					<input type='hidden' id='session' value=<%=session.getValue("sessionid")%>>
				</td>
			...
			<tr>
				...
				<td class='tablelight' colspan=4> 
					<select name='standardscheduleid' id='standardscheduleid' style="width:150px"  validate='Standard Name' >
		...
		<a href="#" class="bt_blue" onClick='loadstudent()'><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.exampass.loadlist")%></strong><span class="bt_blue_r"></span></a>
		</form>		
