...
<script type="text/javascript">
		...
		function loadStandard()
		{
			...
			document.ApplicantSearch.action="RegistrationProcessList_Pre.jsp?reg_sessionid="+document.ApplicantSearch.reg_sessionid.value+"&reg_boardid="+document.ApplicantSearch.reg_boardid.value;
			document.ApplicantSearch.submit();
		...
		function report()
		{
			document.ApplicantSearch.action="RegistrationIssue_Pre1.jsp";
			document.ApplicantSearch.submit();
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
      	<th scope="col" class="rounded-company" width="80%"><%=bundle.getString("label.admisionprocess.Admission")%></th>
            <th scope="col" class="rounded-q4" width="20%"><font color=red>*</font> <%=bundle.getString("label.admisionprocess.MandatoryFields")%></th>        
        </tr>
    </thead>
		...
		<input type=hidden name="req_sessionstatus">
			...
			else if(request.getParameter("reg_sessionid") != null && request.getParameter("reg_boardid") != null)
			{
			 ...
			 yearList = registrationQuery.listYearofSession(instituteid);
			%>
			<tr><td colspan=2><table border=0 align=center id="rounded-corner1" width="100%">
			<tr>
				<td class=tablebold><%=bundle.getString("label.admisionprocess.AcademicYear")%><font color="red">*</font></td>
				<td class=tablebold width="2%">:</td> 
				<td>
				<select name=sessionname class=tablelight onchange=resetBoard(this)>
					...
					for(int s=0;s<yearList.size();s++)
					{
						...
						//out.print("request.getParameter(reg_sessionid)-->"+request.getParameter("reg_sessionid"));
					}
			  ...
			  <tr>
				<td class=tablebold><%=bundle.getString("label.admisionprocess.Board")%> <font color="red">*</font></td>
				<td class=tablebold width="2%">:</td> <td>
			...
			<tr>
				<td class=tablebold><%=bundle.getString("label.admisionprocess.Standard")%> <font color="red">*</font></td>
				<td class=tablebold width="2%">:</td> <td><select name=course class=tablelight onchange=checkStatus()>
			...
			<%
				...
				{
				out.print("<tr><td width='35%' valign=top class='tablebold'>Archival Details</td><td class=tablebold width='2%' valign=top>:</td><td valign=top class='tablelight' ><h2>You can't Admit Student for an Archived Year</h2></td></tr>");	
				}
				else if(request.getParameter("req_sessionstatus") != null && request.getParameter("req_sessionstatus").equals("A"))
				{
				out.print("<tr> <td width='35%' class=tablebold>"+bundle.getString("label.admisionprocess.ApplicationNo")+".<font color='red'>*</font></td><td class=tablebold width='2%'>:</td><td><input type=text name=appno  maxlength=15 class=tablelight ></td></tr>");	
				}
				else 
				{
				out.print("<tr> <td width='35%' class=tablebold>"+bundle.getString("label.admisionprocess.ApplicationNo")+".<font color='red'>*</font></td><td class=tablebold width='2%'>:</td><td><input type=text name=appno  maxlength=15 class=tablelight ></td></tr>");	
     ...
     <a href="#" class="bt_blue" onclick="report()"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.admisionprocess.Report")%></strong><span class="bt_blue_r"></span></a>
     <a href="#" class="bt_blue" onclick="viewFun()"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.admisionprocess.ViewAdmittedApplicants")%></strong><span class="bt_blue_r"></span></a>
     <%
		if(request.getParameter("req_sessionstatus") != null && request.getParameter("req_sessionstatus").equals("A"))
		{
	%>	
     <a href="#" class="bt_green" onclick="admitFun()"><span class="bt_green_lft"></span><strong><%=bundle.getString("button.admisionprocess.Admit")%></strong><span class="bt_green_r"></span></a>
    <% 
		}
		if(request.getParameter("req_sessionstatus") == null || request.getParameter("req_sessionstatus") == "")
		{
	%>	
     <a href="#" class="bt_green" onclick="admitFun()"><span class="bt_green_lft"></span><strong><%=bundle.getString("button.admisionprocess.Admit")%></strong><span class="bt_green_r"></span></a>
    <% 
