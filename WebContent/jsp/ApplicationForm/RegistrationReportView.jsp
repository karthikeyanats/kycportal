...
function goBack()
{
	document.applicationForm.action= "RegistrationReport.jsp";
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.admisionreportviewdatewise.rootpath")%></h2>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
	...
	<tr>
		...
		<td class=tablebold><%=bundle.getString("label.admisionreport.Board")%> </td><td class=tablebold width='2%'>:</td><td class=tablelight><%=request.getParameter("req_boardname") %></td>
	</tr>
	<tr>
		<td class=tablebold width="25%"><%=bundle.getString("label.admisionreport.Standard")%></td><td class=tablebold width='2%'>:</td><td colspan=3 class=tablelight ><%=request.getParameter("standardnamegp") %></td>

	</tr>
	<% 
	if(request.getParameter("req_sessionstatus") != null && request.getParameter("req_sessionstatus").equals("R")){
		out.print("<tr><td class=tablebold width='25%'>Archival Details</td><td class=tablebold width='2%'>:</td><td colspan=3 class=tablelight >This is an Archived Year</td></tr>");
	}
	%>
</table>
</td></tr>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%" align="center">
   <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="10%"><%=bundle.getString("label.admisionreport.SL.No")%></th>
            <th scope="col" class="rounded" width="25%"><%=bundle.getString("label.admisionreport.ApplicantName")%></th>
            <th scope="col" class="rounded" width="22%"><%=bundle.getString("label.admisionreport.ApplicationNo")%></th>
            <th scope="col" class="rounded" width="15%"><%=bundle.getString("label.admisionreport.ReceiptNo")%></th>
            <th scope="col" class="rounded" width="14%"><%=bundle.getString("label.admisionreport.AdmissionFees")%></th>
            <th scope="col" class="rounded-q4" width="16%"><%=bundle.getString("label.admisionreport.AdmissionDate")%></th>
     ...
     try
     {
    	...
    	//System.out.println("applicantList-->"+applicantList);
    	 if(applicantList!=null && applicantList.size()>0)
            ...
            <td><%=applicationNo %></td>
            <td><%=receiptNo %></td>
            <td align=right><%=appCostCurrency %></td>           
            <td align=center><%=dateofcreation %> </td>
        </tr>
				<%
    		 }
			}
