...
<title><%=bundle.getString("label.product.name")%> - <%=bundle.getString("label.applicationreportviewdatewisecons.title")%> | Powered by i-Grandee</title>
...
function goBack()
{
	document.approvalForm.action="ApplicationReport.jsp";
...
//alert("document.approvalForm.appFrom.value-->"+document.approvalForm.appFrom.value)
	...
	document.approvalForm.action="ApplicationReportDatewiseCons.jsp";
	document.approvalForm.submit();
}
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.applicationreportviewdatewisecons.rootpath")%></h2>


<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            <th scope="col" class="rounded-company" width="90%"><%=bundle.getString("button.applicationreport.DatewiseReport")%></th>
...
<tr>
	...
	//out.print("<tr><td class=tablebold width='25%'>"+bundle.getString("label.applicationreport.FromDate")+"</td><td class=tablebold width='2%'>:</td><td><input type='text' name='appFrom' validate='Start Date' size='11' value='' onfocus='this.blur()'><input type='button' value='^' onclick=Calendar_Fun('"+request.getContextPath()+"/js/Calendar/CalendarWindow.html','approvalForm.appFrom','0',event,'Not_Less_Than_CurrentDate',approvalForm.todaydate)></td>");
	//out.print("<td class=tablebold>"+bundle.getString("label.applicationreport.ToDate")+" </td><td class=tablebold width='2%'>:</td><td><input type='text' name='appTo' size='11' value='' validate='End Date' onfocus='this.blur()'><input type='button' value='^' onclick=Calendar_Fun('"+request.getContextPath()+"/js/Calendar/CalendarWindow.html','approvalForm.appTo','0',event,'Not_Less_Than_StartDate',approvalForm.appFrom)></td></tr>");
...
<br>
	...
	<a href="#" class="bt_red" onclick="goBack()"><span class="bt_red_lft"></span><strong><%=bundle.getString("button.applicationreport.Back")%></strong><span class="bt_red_r"></span></a>						
	<a href="#" class="bt_blue" onclick="todayReport()"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.applicationreport.TodayReport")%></strong><span class="bt_blue_r"></span></a>
	<a href="#" class="bt_green" onclick="datewiseReport()"><span class="bt_green_lft"></span><strong><%=bundle.getString("button.applicationreport.Search")%></strong><span class="bt_green_r"></span></a>
		</td>
		</tr>
	</table>	
...
<input type=hidden name='today_date' value='<%=datestr%>'>
