...
<title><%=bundle.getString("label.product.name")%> - <%=bundle.getString("label.applicationdeactivelist.title")%> | Powered by i-Grandee</title>
							...
							if (req.readyState == 4)
							   {
								//   alert(req.status)
							  		 if(req.status == 200)
									 {
											...
											if(finalValue=="1")
											{
												alert("This Application No. Already Exist. You cant Activate Until you Deactivate Existing Application No.")
											}
											else
											{
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2><%=bundle.getString("label.applicationdeactivelist.rootpath")%></h2>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded-company" width="90%"><%=bundle.getString("label.applicationdeactivelist.SelectedInformation")%> </th>
	...
	<tr>
		<td class=tablebold width="25%">&nbsp;&nbsp;&nbsp;&nbsp; <%=bundle.getString("label.applicationdeactivelist.AcademicYear")%></td><td class=tablebold width="2%">:</td><td class=tablelight ><%=request.getParameter("req_sessionname") %></td>
		<td class=tablebold width="25%"><%=bundle.getString("label.applicationdeactivelist.Standard")%></td><td class=tablebold width="2%">:</td><td class=tablelight><%=request.getParameter("standardnamegp") %></td>
	</tr>
	<tr><td class=tablebold>&nbsp;&nbsp;&nbsp;&nbsp; <%=bundle.getString("label.applicationdeactivelist.Board")%> </td><td class=tablebold width="2%">:</td><td  colspan=2  class=tablelight><%=request.getParameter("req_boardname") %></td>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%" align="center">
   <thead>
    	<tr>
        	<th scope="col" class="rounded-company" width="10%"><%=bundle.getString("label.applicationdeactivelist.SL.No")%></th>
            <th scope="col" class="rounded" width="35%"><%=bundle.getString("label.applicationdeactivelist.ApplicantName")%></th>
            <th scope="col" class="rounded" width="25%"><%=bundle.getString("label.applicationdeactivelist.ApplicationNo")%></th>
            <th scope="col" class="rounded" width="25%"><%=bundle.getString("label.applicationdeactivelist.DateofIssue")%></th>
            <th scope="col" class="rounded-q4" width="10%"><%=bundle.getString("label.applicationdeactivelist.Action")%></th>
    ...
    <tfoot>
     ...
     try
     {
    	  ...
    	  applicantList = applicationQuery.loadApplicantList(request.getParameter("req_sessionid"),request.getParameter("req_boardid"),request.getParameter("standardpublishid"),instituteid,"D");
    	 ...
    	 if(applicantList!=null && applicantList.size()>0)
            ...
            {
            %>
            <td><a href="#"  applicationid="<%=applicationId%>" applicantname="<%=applicantName %>" applicationno="<%=applicationNo %>" statusval="Activate" status="A" onclick="statusChangeFun(this)" class="ask"><%=bundle.getString("hlink.applicationdeactivelist.Active")%></a></td>
            <%
            }
            else
            {
	...
	<a href="#" class="bt_green" onclick="addnew_fun()"><span class="bt_green_lft"></span><strong><%=bundle.getString("button.applicationdeactivelist.Addnewitem")%></strong><span class="bt_green_r"></span></a>
    <a href="#" class="bt_red" onclick="viewTrash()"><span class="bt_red_lft"></span><strong><%=bundle.getString("button.applicationdeactivelist.TrashedApplicants")%></strong><span class="bt_red_r"></span></a>
    <a href="#" class="bt_blue" onclick="viewActive()"><span class="bt_blue_lft"></span><strong><%=bundle.getString("button.applicationdeactivelist.ViewActiveApplicants")%> </strong><span class="bt_blue_r"></span></a>
