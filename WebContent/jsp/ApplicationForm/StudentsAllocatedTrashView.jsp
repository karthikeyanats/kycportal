...
<%@ include file="../include/header.jsp" %>
    ...
    <h2>Enrollment / Student Enrollment / Allocation / Student Allocation Trash View</h2>
...
<table id="rounded-corner" border=0 width="100%">
    <thead>
	 ...
	 <%
		...
		registeredList	=	(ArrayList)registrationQuery.listAllocatedDetails(request.getParameter("req_sectionpubid"),"T");
       %>
...
<tr><td class=tablebold width="25%">Standard</td><td class=tablebold width='2%'>:</td><td class=tablelight><%=request.getParameter("standardnamegp") %></td>
<td class=tablebold width="25%">Section</td><td class=tablebold width='2%'>:</td><td class=tablelight><%=request.getParameter("req_sectionname")%></td>
</tr>
</table>
</td></tr>
</table>
...
<table id="rounded-corner" border=0 width="100%">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded" width="20%">Date of Trash</th>              
	...
	if(registeredList.size()!=0)
	{
		for(int i=0;i<registeredList.size();i++)
		{
            ...
            {
            %>	
			<!--  <td><a href="#"  rollno="<%=rollNo%>" studentallocationid="<%=studentallocationId%>" applicantname="<%=applicantName %>" applicationno="<%=applicationNo %>" standardpubid="<%=request.getParameter("standardpublishid")%>" onclick="editApplicant(this)"><img src="<%=request.getContextPath()%>/jsp/kycpanel/images/user_logout.png" alt="" title="" border="0" /></a></td>-->
			<td><a href="#"  studentallocationid="<%=studentallocationId%>"  statusval="Delete" status="X" onclick="statusChangeFun(this)" class="ask"><img src="<%=request.getContextPath()%>/jsp/kycpanel/images/user_logout.png" alt="" title="" border="0" /></a></td>
			<td><a href="#"  studentallocationid="<%=studentallocationId%>"  statusval="Restore" status="A" onclick="statusChangeFun(this)" class="ask"><img src="<%=request.getContextPath()%>/jsp/kycpanel/images/16-undo1.png" alt="" title="" border="0" /></a></td>
            <%
            } 
            else
            {
...
<a href="#" class="bt_green" onclick="viewActive()"><span class="bt_green_lft"></span><strong>Active Applicants</strong><span class="bt_green_r"></span></a>
...
<!-- <input type='hidden' name='stdscheduleid' value='%stdscheduleid%' -->
	...
	<input type='hidden' name='sectionSchedId' value='<%=request.getParameter("req_sectionpubid")%>'>
