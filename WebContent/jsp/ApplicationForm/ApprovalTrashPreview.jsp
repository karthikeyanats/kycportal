...
<title>Know Your Child - Approval Trash Preview | Powered by i-Grandee</title>
...
function goBack()
{
	document.approvalForm.action="ApproveView.jsp";
	document.approvalForm.submit();
}
...
function statusChangeFun(obj)
{
		...
		{
			document.approvalForm.studentapprovalid_hidden.value = obj.getAttribute("studentapprovalid");
			document.approvalForm.statuschange.value = obj.getAttribute("status");
			//alert(obj.getAttribute("studentapprovalid")+"--"+document.approvalForm.studentapprovalid_hidden.value)
			document.approvalForm.action= "approvalStatusSubmit.jsp";
			document.approvalForm.submit();
		}
}
...
<%@ include file="../include/header.jsp" %>
    ...
    <h2>Enrollment / Student Enrollment / Approval <%=request.getParameter("statusval")%> Preview</h2>
...
<table id="rounded-corner" border=0 summary="Department List" width="100%">
    <thead>
    	<tr>
            <th scope="col" class="rounded-company" width="100%">Student Details </th>
            <th scope="col" class="rounded-q4" width="10"></th>        
        </tr>
    </thead>
	...
	<tr>
		<td class=tablebold width="20%">Date of Approval</td><td class=tablebold width='2%'>:</td><td align=left width="35%" colspan=2  class=tablelight><%=request.getParameter("dateofapproval_hidden") %></td>
...
<%
if(!request.getParameter("profilestatus_hidden").equals("InActive"))
{
%>
<br>
<div class=warning_box>Profile entered for this Student.So yo cant <%=request.getParameter("statusval")%>.
</div>
<br><%
}
%>
	...
	<a href="#" class="bt_red" studentapprovalid="<%=request.getParameter("studentapprovalid_hidden")%>"  statusval="<%=request.getParameter("statusval")%>" status="<%=request.getParameter("statuschange")%>" onclick="statusChangeFun(this)"><span class="bt_red_lft"></span><strong><%=request.getParameter("statusval")%></strong><span class="bt_red_r"></span></a>
	<br><%
	}
	%>
...
<input type='hidden' name='studentapprovalid' value='<%=request.getParameter("studentapprovalid_hidden")%>'>
<input type='hidden' name='studentapprovalid_hidden' value='<%=request.getParameter("studentapprovalid_hidden")%>'>
